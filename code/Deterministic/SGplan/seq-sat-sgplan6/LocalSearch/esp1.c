...
float makespan = dis_BIG_INT, max_time = dis_BIG_INT, violation = dis_BIG_INT, total_time = dis_BIG_INT;
float schedule[1000];
...
MutexType mutex_op(int op1, float s1, int op2, float s2)
{
	...
	e1 = s1 + dis_gef_conn[ef1].duration;
	e2 = s2 + dis_gef_conn[ef2].duration;	
	...
	for (i=x;i<n;i++)
	{
		...
		for (j=0;j<i;j++)
		{
			...
			if (dis_depend_efs(ef1, ef2))
				if (time[i] < time[j] + dis_gef_conn[ef2].duration + 0.003)
					time[i] = time[j] + dis_gef_conn[ef2].duration + 0.003;
		}
		if (GpG.SearchModal > -1000 && GpG.is_til)
		{
...
void copy_final_plan(void)
{
	...
	for (i=0;i<num_subgoal;i++)
		...
		{
			d = dis_gef_conn[gop_conn[subplan_ops[i][j]].E[0]].duration;
							...
							for (u=0;u<dis_gef_conn[ef].num_PC;u++)
								for (v=0;v<dis_gef_conn[ef2].num_D;v++)
									if (dis_gef_conn[ef2].D[v] == dis_gef_conn[ef].PC[u])
									{
										...
										if (scheduled_time[j][l] + dis_gef_conn[ef2].duration > t)
										{
											objs[i][j]++;
							...
							for (u=0;u<dis_gef_conn[ef2].num_PC;u++)
								for (v=0;v<dis_gef_conn[ef].num_D;v++)
									if (dis_gef_conn[ef].D[v] == dis_gef_conn[ef2].PC[u])
									{
										...
										if (scheduled_time[i][k] + dis_gef_conn[ef].duration > t)
										{
											objs[i][j]++;
