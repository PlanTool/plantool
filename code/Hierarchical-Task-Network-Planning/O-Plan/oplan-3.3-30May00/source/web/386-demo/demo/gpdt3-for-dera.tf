;;; File: gpdt3-for-dera.tf
;;; Contains: TF for the Pacifica COA matrix demo modified for DERA demo
;;; Created: John Levine, June 1997
;;; Modified for DERA: Peter Jarvis, June 1999
;;; Updated: Thu Jun 24 16:15:01 1999 by Jeff Dalton
;;; Copyright: (c) 1997, AIAI, University of Edinburgh

;;; Modified GPDT domain for DERA TG10 Project on integrating simulators
;;; with AI planners. Domain was modified to provide the AI planner
;;; element of DERA's July 99 demonstration of the Query and Data Manager
;;; which sits between an AI planner and a suite of simulators.

types city               = (Abyss Barnacle Calypso Delta Exodus),
      fast_vehicle       = (4_Ton_Bedford_1 4_Ton_Leyland_1 RB_44_1),
      slow_armored_vehicle = (US_M2_A2_Bradley_1 US_M2_A2_Bradley_2
                              US_M2_A2_Bradley_3 UK_MCV_80_1 UK_MCV_8_2),
      fleet_of_trucks    = (British_Fleet_1 Indian_Fleet_1),
      helicopter         = (UK_Chinook1 US_BlackHalk1),
      repair_team        = (US_RT_1 UK_RT_1),
      repair_equipment   = (RE1 RE2),
      medical_team       = (US_PJ_1 UK_MT_1 UK_MT_2),
      medical_equipment  = (Field_Hosptial_1
                            First_Aid_Center_1 First_Aid_Center_2),
      evacuation_team    = (ET1 ET2),
      bomb_squad         = (UK_BS_1 US_BS_1),
      building_team      = (BT1 BT2),
      building_equipment = (BE1 BE2),
      commander          = (CDR1),
      engineer           = (ENG1),
...
;;; Dummy task to make sure all there are levels for all the initial
;;; effects we'll use.  This is actually needed only for road_status,
;;; because otherwise it doesn't appear anywhere in this file.
...
task effects_dummy;
  ...
  effects
    {weather} = clear at 1,
    {road_status ??} = open at 1,
    {location ??} = Delta at 1,
    {available ??} = true at 1,
    {empty_vehicle ??} = true at 1;
end_task;
...
schema evacuate_injured_using_helicopters;
  vars ?weather = ?{type flying_weather},
       ?v1 = ?{type helicopter},
       ?v2 = ?{type helicopter},
  ...
  nodes
    ...
    4 action {load_injured ?mt ?v2 ?at},
    5 action {move_injured_by_air ?v2 ?at Delta};
  orderings
    1 ---> 3, 2 ---> 3, 3 ---> 4, 4 ---> 5;
  conditions
    only_use_if {weather} = ?weather;
end_schema;

schema evacuate_injured_using_helicopters_and_fast_truck_ground_vehicles;
  vars ?weather = ?{type flying_weather},
       ?v1 = ?{type helicopter},
       ?v2 = ?{type fast_vehicle},
       ?mt = ?{type medical_team},
       ?me = ?{type medical_equipment},
       ?at = ?{type city};
  expands {evacuate_injured ?at};
  nodes
    1 action {transport_by_air ?v1 ?mt ?at},
    2 action {fast_transport ?v2 ?me ?at},
    3 action {deal_with_injured ?mt ?me ?at},
    4 action {load_injured ?mt ?v2 ?at},
    5 action {move_injured_by_road ?v2 ?at Delta};
  orderings
    1 ---> 3, 2 ---> 3, 3 ---> 4, 4 ---> 5;
  conditions
    only_use_if {weather} = ?weather;
...
schema evacuate_injured_using_helicopters_and_slow_armored_ground_vehicles;
  vars ?weather = ?{type flying_weather},
       ?v1 = ?{type helicopter},
       ?v2 = ?{type slow_armored_vehicle},
       ?mt = ?{type medical_team},
       ?me = ?{type medical_equipment},
       ?at = ?{type city};
  expands {evacuate_injured ?at};
...
schema evacuate_injured_using_fast_truck_ground_vehicles;
  vars ?v1 = ?{type fast_vehicle},
       ?v2 = ?{type fast_vehicle},
       ?mt = ?{type medical_team},
       ?me = ?{type medical_equipment},
       ?at = ?{type city};
  expands {evacuate_injured ?at};
  nodes
    1 action {fast_transport ?v1 ?mt ?at},
    2 action {fast_transport ?v2 ?me ?at},
    3 action {deal_with_injured ?mt ?me ?at},
    4 action {load_injured ?mt ?v2 ?at},
    5 action {move_injured_by_road ?v2 ?at Delta};
...
schema evacuate_injured_using_fast_trucks_and_slow_armored_ground_vehicles;
  vars ?v1 = ?{type fast_vehicle},
       ?v2 = ?{type slow_armored_vehicle},
       ?mt = ?{type medical_team},
       ?me = ?{type medical_equipment},
...
schema evacuate_injured_using_slow_armored_ground_vehicles;
  vars ?v1 = ?{type slow_armored_vehicle},
       ?v2 = ?{type slow_armored_vehicle},
       ?mt = ?{type medical_team},
       ?me = ?{type medical_equipment},
       ?at = ?{type city};
...
schema evacuate_population_using_a_helicopter;
  vars ?weather = ?{type flying_weather},
       ?v1 = ?{type helicopter},
  ...
  nodes
    1 action {transport_by_air ?v1 ?et ?at},
...
schema evacuate_population_using_a_fast_truck_ground_vehicle;
  vars ?v1 = ?{type fast_vehicle},
       ?v2 = ?{type fleet_of_trucks},
       ?et = ?{type evacuation_team},
       ?at = ?{type city};
  expands {evacuate_population ?at};
  nodes
    1 action {fast_transport ?v1 ?et ?at},
    2 action {send_fleet_of_trucks ?v2 ?at},
    3 action {load_population ?et ?v2 ?at},
    4 action {move_population ?v2 ?at Delta};
  orderings
...
schema evacuate_population_using_a_slow_armored_vehicle;
  vars ?v1 = ?{type slow_armored_vehicle},
       ?v2 = ?{type fleet_of_trucks},
       ?et = ?{type evacuation_team},
       ?at = ?{type city};
...
schema evacuate_with_medical_team_using_helicopters;
  vars ?weather = ?{type flying_weather},
       ?v1 = ?{type helicopter},
       ?v2 = ?{type helicopter},
  ...
  nodes 
    ...
    5 action {load_injured ?mt ?v2 ?at},
    6 action {load_population ?mt ?v3 ?at},
    7 action {move_injured_by_air ?v2 ?at Delta},
...
schema evacuate_with_medical_team_using_helicopters_and_fast_truck_ground_vehicles;
  vars ?weather = ?{type flying_weather},
       ?v1 = ?{type helicopter},
       ?v2 = ?{type fast_vehicle},
       ?v3 = ?{type fleet_of_trucks},
  ...
  nodes 
    ...
    5 action {load_injured ?mt ?v2 ?at},
    6 action {load_population ?mt ?v3 ?at},
    7 action {move_injured_by_road ?v2 ?at Delta},
    8 action {move_population ?v3 ?at Delta};
  orderings
    1 ---> 4, 2 ---> 4, 3 ---> 6,
    4 ---> 5, 5 ---> 6,
...
schema evacuate_with_medical_team_using_helicopters_and_slow_armored_ground_vehicles;
  vars ?weather = ?{type flying_weather},
       ?v1 = ?{type helicopter},
       ?v2 = ?{type slow_armored_vehicle},
       ?v3 = ?{type fleet_of_trucks},
...
schema evacuate_with_medical_team_using_fast_truck_ground_vehicles;
  vars ?v1 = ?{type fast_vehicle},
       ?v2 = ?{type fast_vehicle},
       ?v3 = ?{type fleet_of_trucks},
       ?mt = ?{type medical_team},
...
schema evacuate_with_medical_team_using_fast_trucks_and_slow_armored_ground_vehicles;
  vars ?v1 = ?{type fast_vehicle},
       ?v2 = ?{type slow_armored_vehicle},
       ?v3 = ?{type fleet_of_trucks},
       ?mt = ?{type medical_team},
...
schema evacuate_with_medical_team_using_slow_armored_ground_vehicles;
  vars ?v1 = ?{type slow_armored_vehicle},
       ?v2 = ?{type slow_armored_vehicle},
       ?v3 = ?{type fleet_of_trucks},
       ?mt = ?{type medical_team},
...
schema send_medical_supplies_using_a_helicopter;
  vars ?weather = ?{type flying_weather},
       ?v1 = ?{type helicopter},
  ...
  nodes
    1 action {transport_by_air ?v1 ?ms ?at};
  conditions
    only_use_if {weather} = ?weather,
...
schema send_medical_supplies_using_a_fast_truck_ground_vehicle;
  vars ?v1 = ?{type fast_vehicle},
       ?ms = ?{type medical_supplies},
       ?at = ?{type city};
  expands {send_medical_supplies ?at};
  nodes
    1 action {fast_transport ?v1 ?ms ?at};
  conditions
    unsupervised {available ?ms} = true;
  effects
    {available ?ms} = false at begin_of self;
...
schema send_medical_supplies_using_a_slow_armored_vehicle;
  vars ?v1 = ?{type slow_armored_vehicle},
       ?ms = ?{type medical_supplies},
       ?at = ?{type city};
  expands {send_medical_supplies ?at};
  nodes
    1 action {slow_transport ?v1 ?ms ?at};
  conditions
    unsupervised {available ?ms} = true;
  effects
    {available ?ms} = false at begin_of self;
...
schema send_emergency_food_using_a_helicopter;
  vars ?weather = ?{type flying_weather},
       ?v1 = ?{type helicopter},
  ...
  nodes
    1 action {transport_by_air ?v1 ?ef ?at};
  conditions
    only_use_if {weather} = ?weather,
...
schema send_emergency_food_using_a_fast_truck_ground_vehicle;
  vars ?v1 = ?{type fast_vehicle},
       ?ef = ?{type emergency_food},
       ?at = ?{type city};
  expands {send_emergency_food ?at};
  nodes
    1 action {fast_transport ?v1 ?ef ?at};
  conditions
    unsupervised {available ?ef} = true;
  effects
    {available ?ef} = false at begin_of self;
...
schema send_emergency_food_using_a_slow_armored_vehicle;
  vars ?v1 = ?{type slow_armored_vehicle},
       ?ef = ?{type emergency_food},
       ?at = ?{type city};
  expands {send_emergency_food ?at};
  nodes
    1 action {slow_transport ?v1 ?ef ?at};
  conditions
    unsupervised {available ?ef} = true;
  effects
    {available ?ef} = false at begin_of self;
...
schema send_medical_team_using_helicopters;
  vars ?weather = ?{type flying_weather},
       ?v1 = ?{type helicopter},
       ?v2 = ?{type helicopter},
       ?mt = ?{type medical_team},
...
schema send_medical_team_using_helicopters_and_fast_truck_ground_vehicles;
  vars ?weather = ?{type flying_weather},
       ?v1 = ?{type helicopter},
       ?v2 = ?{type fast_vehicle},
       ?mt = ?{type medical_team},
...
schema send_medical_team_using_helicopters_and_slow_armored_ground_vehicles;
  vars ?weather = ?{type flying_weather},
       ?v1 = ?{type helicopter},
       ?v2 = ?{type slow_armored_vehicle},
       ?mt = ?{type medical_team},
...
schema send_medical_team_using_fast_truck_ground_vehicles;
  vars ?v1 = ?{type fast_vehicle},
       ?v2 = ?{type fast_vehicle},
       ?mt = ?{type medical_team},
       ?me = ?{type medical_equipment},
...
schema send_medical_team_using_fast_trucks_and_slow_armored_ground_vehicles;
  vars ?v1 = ?{type fast_vehicle},
       ?v2 = ?{type slow_armored_vehicle},
       ?mt = ?{type medical_team},
       ?me = ?{type medical_equipment},
...
schema send_medical_team_using_slow_armored_ground_vehicles;
  vars ?v1 = ?{type slow_armored_vehicle},
       ?v2 = ?{type slow_armored_vehicle},
       ?mt = ?{type medical_team},
       ?me = ?{type medical_equipment},
...
schema repair_gas_leak_using_helicopters;
  vars ?weather = ?{type flying_weather},
       ?v1 = ?{type helicopter},
       ?v2 = ?{type helicopter},
       ?tm = ?{type repair_team},
  ...
  nodes
    1 action {transport_by_air ?v1 ?tm ?at},
    2 action {transport_by_air ?v2 ?eq ?at},
...
schema repair_gas_leak_using_helicopters_and_fast_truck_ground_vehicles;
  vars ?weather = ?{type flying_weather},
       ?v1 = ?{type helicopter},
       ?v2 = ?{type fast_vehicle},
       ?tm = ?{type repair_team},
  ...
  nodes
    1 action {transport_by_air ?v1 ?tm ?at},
    2 action {fast_transport ?v2 ?eq ?at},
    3 action {fix_gas_leak ?tm ?eq ?at},
    4 action {restart_power_station ?tm ?at};
  orderings
    1 ---> 3, 2 ---> 3, 3 ---> 4;
...
schema repair_gas_leak_using_helicopters_and_slow_armored_ground_vehicles;
  vars ?weather = ?{type flying_weather},
       ?v1 = ?{type helicopter},
       ?v2 = ?{type slow_armored_vehicle},
       ?tm = ?{type repair_team},
...
schema repair_gas_leak_using_fast_truck_ground_vehicles;
  vars ?v1 = ?{type fast_vehicle},
       ?v2 = ?{type fast_vehicle},
       ?tm = ?{type repair_team},
       ?eq = ?{type repair_equipment},
  ...
  expands {repair_gas_leak ?at};
  nodes
    1 action {fast_transport ?v1 ?tm ?at},
    2 action {fast_transport ?v2 ?eq ?at},
    3 action {fix_gas_leak ?tm ?eq ?at},
    4 action {restart_power_station ?tm ?at};
  orderings
...
schema repair_gas_leak_using_fast_trucks_and_slow_armored_ground_vehicles;
  vars ?v1 = ?{type fast_vehicle},
       ?v2 = ?{type slow_armored_vehicle},
       ?tm = ?{type repair_team},
       ?eq = ?{type repair_equipment},
...
schema repair_gas_leak_using_slow_armored_ground_vehicles;
  vars ?v1 = ?{type slow_armored_vehicle},
       ?v2 = ?{type slow_armored_vehicle},
       ?tm = ?{type repair_team},
       ?eq = ?{type repair_equipment},
...
schema defuse_terrorist_bomb_using_a_helicopter;
  vars ?weather = ?{type flying_weather},
       ?v1 = ?{type helicopter},
       ?bs = ?{type bomb_squad},
       ?at = ?{type city};
  ...
  nodes
    1 action {transport_by_air ?v1 ?bs ?at},
...
schema defuse_terrorist_bomb_using_a_fast_truck_ground_vehicle;
  vars ?v1 = ?{type fast_vehicle},
       ?bs = ?{type bomb_squad},
       ?at = ?{type city};
  expands {defuse_terrorist_bomb ?at};
  nodes
    1 action {fast_transport ?v1 ?bs ?at},
    2 action {defuse_bomb ?bs ?at};
  orderings
    1 ---> 2;
end_schema;

schema defuse_terrorist_bomb_using_a_slow_armored_vehicle;
  vars ?v1 = ?{type slow_armored_vehicle},
       ?bs = ?{type bomb_squad},
       ?at = ?{type city};
  expands {defuse_terrorist_bomb ?at};
  nodes
    1 action {slow_transport ?v1 ?bs ?at},
    2 action {defuse_bomb ?bs ?at};
  orderings
    1 ---> 2;
end_schema;

schema build_emergency_housing_using_helicopters;
  vars ?weather = ?{type flying_weather},
       ?v1 = ?{type helicopter},
       ?v2 = ?{type helicopter},
       ?v3 = ?{type fleet_of_trucks},
  ...
  nodes
    1 action {transport_by_air ?v1 ?bt ?at},
    2 action {transport_by_air ?v2 ?be ?at},
...
schema build_emergency_housing_using_helicopters_and_slow_armored_ground_vehicles;
  vars ?weather = ?{type flying_weather},
       ?v1 = ?{type helicopter},
       ?v2 = ?{type slow_armored_vehicle},
       ?v3 = ?{type fleet_of_trucks},
...
schema build_emergency_housing_using_slow_armored_ground_vehicles;
  vars ?v1 = ?{type slow_armored_vehicle},
       ?v2 = ?{type slow_armored_vehicle},
       ?v3 = ?{type fleet_of_trucks},
       ?bt = ?{type building_team},
...
schema repair_power_station_turbine_using_helicopters;
  vars ?weather = ?{type flying_weather},
       ?v1 = ?{type helicopter},
       ?v2 = ?{type helicopter},
       ?tm = ?{type repair_team},
...
schema repair_power_station_turbine_using_fast_truck_ground_vehicles;
  vars ?v1 = ?{type fast_vehicle},
       ?v2 = ?{type fast_vehicle},
       ?tm = ?{type repair_team},
       ?eq = ?{type repair_equipment},
...
schema repair_power_station_turbine_using_slow_armored_ground_vehicles;
  vars ?v1 = ?{type slow_armored_vehicle},
       ?v2 = ?{type slow_armored_vehicle},
       ?tm = ?{type repair_team},
       ?eq = ?{type repair_equipment},
...
schema provide_immediate_assistance_using_a_helicopter;
  vars ?weather = ?{type flying_weather},
       ?v  = ?{type helicopter},
       ?c  = ?{type commander},
       ?a  = ?{type city};
  ...
  nodes
    1 action {transport_by_air ?v ?c ?a},
...
schema provide_immediate_assistance_using_a_fast_truck_ground_vehicle;
...
schema provide_immediate_assistance_using_a_slow_armored_vehicle;
  vars ?v = ?{type slow_armored_vehicle},
       ?c = ?{type commander},
       ?a = ?{type city};
  expands {provide_immediate_assistance ?a};
  nodes
    1 action {slow_transport ?v ?c ?a},
    2 action {deal_with_crisis ?c ?a};
  orderings
    1 ---> 2;
end_schema;

schema shut_down_power_station_using_a_helicopter;
  vars ?weather = ?{type flying_weather},
       ?v  = ?{type helicopter},
       ?e  = ?{type engineer},
       ?a  = ?{type city};
  ...
  nodes
    1 action {transport_by_air ?v ?e ?a},
...
schema shut_down_power_station_using_a_fast_truck_ground_vehicle;
  vars ?v = ?{type fast_vehicle},
       ?e = ?{type engineer},
       ?a = ?{type city};
  expands {shut_down_power_station ?a};
...
schema shut_down_power_station_using_a_slow_armored_vehicle;
  vars ?v = ?{type slow_armored_vehicle},
       ?e = ?{type engineer},
       ?a = ?{type city};
  expands {shut_down_power_station ?a};
  nodes
    1 action {slow_transport ?v ?e ?a},
    2 action {shut_down_power ?e ?a};
  orderings
    1 ---> 2;
end_schema;
...
schema transport_in_trucks;
  ...
  use ?v across self,
      ?i across self;
  time_windows duration self = 4 hours;
end_schema;
