...
public:
  ...
  unsigned oper( void ) const { return operator_; }
  const Application& application( void ) const { return( *application_ ); }
  const Expression& expression( void ) const { return( *expr_ ); }
...
public:
  ...
  static void register_use( const Effect* e )
    {
#ifdef MEM_DEBUG
      if(e) std::cerr << "<eff>: inc-ref-count " << e << " = " 
		      << e->ref_count_+1 << std::endl;
#endif
      if( e ) e->ref_count_++;
    }
  static void unregister_use( const Effect* e )
    {
#ifdef MEM_DEBUG
      if(e) std::cerr << "<eff>: dec-ref-count " << e << " = " 
		      << e->ref_count_-1 << std::endl;
#endif
      if( e && (--e->ref_count_ == 0) ) 
	{
  ...
  virtual effect_t *translate( void ) const = 0;
  ...
  virtual const Effect& rewrite( std::map<const StateFormula*,
				 const Atom*> &hash ) const = 0;
};
...
public:
  ...
  virtual effect_t* translate( void ) const;
...
public:
  ...
  bool add_outcome( double p, const Effect& effect );
