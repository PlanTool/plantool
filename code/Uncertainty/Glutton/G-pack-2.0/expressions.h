...
struct ApplicationLess : public std::binary_function<const Application*,const Application*,bool>
  {
  public:
...
class ValueMap : public std::map<const Application*, double, ApplicationLess> {};
...
public:
  ...
  virtual double value( const ValueMap& values ) const = 0;
...
public:
  Value( const double& value ) { value_ = value; }
  const double& value( void ) const { return( value_ ); }
  virtual double value( const ValueMap& values ) const;
