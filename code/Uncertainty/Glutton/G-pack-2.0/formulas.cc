...
{
  //std::cout<<"Flattening constant!!"<<std::endl;
  StateFormula::register_use( this );
  return( *this );
}
		     ...
		     const problem_t& problem ) const
{
  ...
  for( TermList::const_iterator ti = terms_.begin(); ti != terms_.end(); ++ti )
    {
      SubstitutionMap::const_iterator si = (is_variable( *ti )?subst.find( *ti )
					                          :subst.end());
      if( si != subst.end() )
	{
	  inst_terms.push_back( (*si).second );
...
const StateFormula& 
Comparison::instantiation( const SubstitutionMap& subst, 
			                         const problem_t& problem) const
{
  FunctionTable* pft = new FunctionTable();
  const FunctionTable& ft = *pft;
  TermTable* ptt = new TermTable();
  const TermTable&  tt = *ptt;
  const Expression& inst_expr1 = expr1().instantiation( subst, problem );
  const Value& t = (const Value&)*expr2_;
  t.print(std::cout, ft, tt);
...
Negation::make_negation( const StateFormula& formula )
{
  if( formula.tautology() )
    {
      std::cout<<"Negation is a tautology!!"<<std::endl;
			 ...
			 const problem_t& problem) const
{
  ...
  if( inst_negand.contradiction() )
    ...
    {
      //std::cout<<"Something wrong with negation instantiation!!"<<std::endl;
      StateFormula::unregister_use( &inst_negand );
      StateFormula::register_use( &FALSE );
      return( FALSE );
    }
