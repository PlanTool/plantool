...
public:
  ...
  static void register_use( const problem_t *p )
    {
#ifdef MEM_DEBUG
      if(p) std::cerr << "<pro>: inc-ref-count " << p << " = " 
		      << p->ref_count_+1 << std::endl;
#endif
      if( p ) ++p->ref_count_;
    }
  static void unregister_use( const problem_t *p )
    {
#ifdef MEM_DEBUG
      if(p) std::cerr << "<pro>: dec-ref-count " << p << " = " 
		      << p->ref_count_-1 << std::endl;
#endif
  ...
  void add_init_fluent( const Application &application, const double &value );
  ...
  void set_horizon( size_t horizon ) { horizon_ = horizon; }
  ...
  size_t horizon( void ) const { return horizon_; }
  double max_reward(void) const { return max_reward_; } 
  double discount_factor( void ) const { return discount_factor_; } 
  const ActionList& actions( void ) const { return( actions_ ); }
  size_t noop_idx( void ) const { return noopIdx_; }
  ...
  /*
    Returns the maximum immediate reward any action can yield in the given
    state.
  */
  double eval_all_actions(const state_t &state) const;

  /*
    Returns the immediate reward the given action can yield in the given state.
  */
  double eval_action(action_t &action, const state_t &state) const;
  void initial_states( std::pair<state_t*,double> *list ) const;
