...
void 
atomList_t::full_print( std::ostream &os, const problem_t* problem ) const
{
  if( size() == 0 )
    os << "[ <empty> ]";
  else
    {
      ...
      for( size_t i = 0; i < size(); ++i )
	{
	  const Atom *at = problem_t::atom_inv_hash_get(atom( i ));
	  os << atom( i ) << (atom( i )%2?":(not ":":");
	  problem->print( os, *at );
	  if ( atom( i ) % 2 ) os <<")"; 
	  if( i + 1 < size() ) os << " ";
	}
...
void 
atomListList_t::full_print( std::ostream &os, const problem_t *problem ) const
{
  os << "[";
  for( size_t i = 0; i < size(); ++i )
    {
      atom_list( i ).full_print( os, problem );
      if( i + 1 < size() ) os << ",";
    }
  os << "]";
}
