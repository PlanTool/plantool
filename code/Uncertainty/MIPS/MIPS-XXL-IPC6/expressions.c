...

{
  ...
  for ( a = gactions; a; a = a->next ) {
    ...
    */
    if(isDurative==TRUE){
     if ( !is_linear_expression(a->action_duration) )
       return FALSE;
    }
    /* effects
     */
...
/*
  for ( i = 0; i < gnum_operators; i++ ) {
	...
	continue;
      ...
      print_lnf_Action( a );*/
      /* print_LnfExpNode(a->lnf_duration);
...

{
  ...
  while ( a ) {
     ...
     */
    if(isDurative==TRUE){
    if ( !translate_divisions( &(a->action_duration) ) ){
          printf("\n\nff: division by zero in duration. no plan will solve it.\n\n");
      exit( 1 );
    }
      push_multiplications_down( &(a->action_duration) );
    }
  ...
  for ( a = gactions; a; a = a->next ) {
    ...
    if(isDurative)
         a->lnf_preconds_lh_t = ( int * ) calloc( MAX_LNF_COMPS, sizeof( int ) );
	...
	a->numeric_preconds_comp[i] = LEQ;
      ...
      if(isDurative)
             a->lnf_preconds_lh_t[a->num_lnf_preconds]= a->numeric_preconds_lh_t[i];
    ...
    if(isDurative==TRUE){
      ...
      a->lnf_duration = new_LnfExpNode();
      lnum_F = 0;
      lc = 0; 
      a->lnf_duration->num_pF = 0;
      a->lnf_duration->num_nF = 0;
      a->lnf_duration->c = 0;
      collect_normalized_locals2( a->action_duration, TRUE );
     lnf = a->lnf_duration;
      for ( j = 0; j < lnum_F; j++ ) {
       if ( lC[j] == 0 ) continue;
        if ( lC[j] > 0 ) {
    ...
    for ( i = 0; i < a->num_effects; i++ ) {
      ...
      if(isDurative)
           e->lnf_conditions_lh_t = ( int * ) calloc( MAX_LNF_COMPS, sizeof( int ) );
	...
	e->lnf_conditions_lh[e->num_lnf_conditions] = new_LnfExpNode();
        ...
        if(isDurative)
             e->lnf_conditions_lh_t[e->num_lnf_conditions]= e->numeric_conditions_lh_t[i];
	...
	continue;
      ...
      if(isDurative)
             e->lnf_effects_fl_t = ( int * ) calloc( MAX_LNF_EFFS, sizeof( int ) );
	...
	if(isDurative==TRUE)
           e->lnf_effects_fl_t[e->num_lnf_effects] = e->numeric_effects_fl_t[j];
...

{
  ...
  switch ( n->connective ) {
  case AD:
    collect_normalized_locals2( n->leftson, positive );
    collect_normalized_locals2( n->rightson, positive );
    break;
  case SU:
    collect_normalized_locals2( n->leftson, positive );
    collect_normalized_locals2( n->rightson, negative );
    break;
  case MINUS:
    collect_normalized_locals2( n->son, negative );
    break;
  case NUMBER:
    if ( positive ) {
      lc += n->value;
  ...
  for ( a = gactions; a; a = a->next ) {
	...
	*fl = a->lnf_preconds_lh[i]->nF[0];
	return TRUE;
      ...
      if(isDurative==TRUE){
        if ( a->lnf_duration->num_nF > 0 ) {
	*fl = a->lnf_duration->nF[0];
	return TRUE;
      }
      }
...

{
  ...
  for ( a = gactions; a; a = a->next ) {
    ...
    if(isDurative){
        ...
        for ( j = 0; j < a->lnf_duration->num_nF; j++ ) {
	if ( a->lnf_duration->nF[j] == fl ) break;
      }
  
      if ( j <a->lnf_duration->num_nF ) {
      if ( a->lnf_duration->num_pF == MAX_LNF_F ) {
	printf("\n\nincrease MAX_LNF_F! currently %d\n\n", MAX_LNF_F);
	exit( 1 );
      }
      a->lnf_duration->pF[a->lnf_duration->num_pF] = lminus_fluent[fl];
      a->lnf_duration->pC[a->lnf_duration->num_pF++] = a->lnf_duration->nC[j];
      for ( k = j; k < a->lnf_duration->num_nF - 1; k++ ) {
	a->lnf_duration->nF[k] = a->lnf_duration->nF[k+1];
	a->lnf_duration->nC[k] = a->lnf_duration->nC[k+1];
      }
      a->lnf_duration->num_nF--;
      }
    }
 ...
 */
    /*  if(isDurative==TRUE){
       ...
       if ( a->lnf_duration->num_pF > 1 ||
	   (a->lnf_duration->num_pF == 1 && a->lnf_duration->pC[0] != 1) ) {
	for ( j = 0; j < a->lnf_duration->num_pF; j++ ) {
	  lF[j] = a->lnf_duration->pF[j];
	  lC[j] = a->lnf_duration->pC[j];
	}
	lnum_F = a->lnf_duration->num_pF;
	return TRUE;
      }
 ...
 */
    if(isDurative==TRUE){
     ...
     if ( is_artificial_fluent( a->lnf_duration ) ) {
     
      a->lnf_duration->pF[0] = gnum_relevant_fluents - 1;
      a->lnf_duration->pC[0] = 1;
      a->lnf_duration->num_pF = 1;
     }
    }
...

{
  ...
  for ( a = gactions; a; a = a->next ) {
	...
	if(!fl_isSame_to_isViolated(e->lnf_effects_fl[j]))/*effect isn't is-violated-g*/
