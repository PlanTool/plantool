...
/*hhhhhh*/
    ...
    for ( w = ggoal->sons; w; w = w->next ) {
     ...
     /*------------duration-affectation------------------*/

	if(isDurative==TRUE){

	    tmp->action_duration = new_ExpNode( NUMBER );
           tmp->action_duration->value = 0;
          }
    
   /*-------end--of-----duration-affectation------------------*/
	...
	tmp->numeric_preconds_rh = ( ExpNode_pointer * ) calloc( mn, sizeof( ExpNode_pointer ) );
             ...
             if(isDurative){
              ...
              tmp->preconds_t = ( int * ) calloc( m, sizeof( int ) );
              tmp->numeric_preconds_lh_t = ( int * ) calloc( mn, sizeof( int ) );
	    }
	...
	if ( w->connective == ATOM ) {
	  tmp->preconds = ( int * ) calloc( 1, sizeof( int ) );
          ...
          if(isDurative){
              tmp->preconds_t = ( int * ) calloc( 1, sizeof( int ) );
            }
	...
	if ( w->connective == COMP ) {
	  ...
	  tmp->numeric_preconds_lh = ( ExpNode_pointer * ) calloc( 1, sizeof( ExpNode_pointer ) );
          ...
          if(isDurative)
           tmp->numeric_preconds_lh_t = ( int * ) calloc( 1, sizeof(int ) );
      ...
      if(isDurative)
          tmp->effects[0].adds_t = ( int * ) calloc( 1, sizeof( int ) );

      tmp->effects[0].adds[0] = gnum_relevant_facts - 1;

       if(isDurative){
                 tmp->effects[0].adds_t[0] = 1;
	    }
...
/*---- Routine to find intersection time windows Literals ----------*/


TimeWindows *calculte_inter_between_Two_interval_list(TimeWindows *tw1, TimeWindows *tw2){

 
    TimeWindows  *timewhelp1 = NULL;
    TimeWindows  *timewhelp2 = NULL;
    
    while(tw1!=NULL){
	if(tw2!=NULL){
           timewhelp2 = calculate_inter_between_interval_und_literal(tw1, tw2);
           if(timewhelp2!=NULL)
            attach_on_list(timewhelp1, timewhelp2);
...
TimeWindows *calculate_inter_between_interval_und_literal(TimeWindows *interval_literal, TimeWindows *tw){
   
    TimeWindows *help1 = NULL;
    TimeWindows *help2 = NULL;
    while(tw!=NULL){
	help2 = calculate_inter_between_two_interval(interval_literal, tw);
        if(help2!=NULL)
	    attach_on_list(help1, help2);
...
TimeWindows *calculate_inter_between_two_interval(TimeWindows *interval1, TimeWindows *interval2){


    TimeWindows *interval = NULL;
    float a, b;
...
TimeWindows *attach_on_list(TimeWindows *list1, TimeWindows *list2){
...
Bool same_TokenList( TokenList *list1, TokenList *list2 )

{

  TokenList *i_tl1, *i_tl2;

  i_tl1 = list1;
  i_tl2 = list2;

  if ((NULL==i_tl1) || (NULL==i_tl2 )) {
      return FALSE;
  }
  if ((NULL!=i_tl1) && (NULL!=i_tl2 )) {
   
    if(strcmp( i_tl1->item, i_tl2->item ) == SAME){
         i_tl1 = i_tl1->next;
         i_tl2 = i_tl2->next;
    }
    else
...

{
  ...
  Fact *f;
  TokenList *fact_element, *current, *head;
  TimeWindows *time_windows_help;
  
  int num_interv, l, k = 0;

  a = gactions; p = NULL;
   
  if(isTempTimeWindows){
     twpredicate = (TokenList_pointer *) calloc(MAX_PREDICATES ,sizeof(TokenList_pointer) );
   }

   if(isDomainPreference){
     relevant_pref_preconds_predicate = (int *) calloc(MAX_PREDICATES ,sizeof(int) );
     gnum_pref_preconds_predicate = 0;
   }
...
/*------------duration-affectation------------------*/
	...
	if(isDurative==TRUE){
           ...
           a->action_duration = ( ExpNode * )
	  calloc( 1, sizeof( ExpNode ) );
	   a->action_duration = copy_Exp(no->normoperator_duration) ;
	   instantiate_exp_by_action( &(a->action_duration), a );
           set_relevants_in_exp(&(a->action_duration));
          }
...
/*-------end--of-----duration-affectation------------------*/
	...
	a->preconds = ( int * ) calloc( no->num_preconds, sizeof( int ) );
        ...
        if(isDurative)
             a->preconds_t = ( int * ) calloc( no->num_preconds, sizeof( int ) );
      }
      
      /*for pref preconds
      **/
      if ( no->num_pref_preconds > 0 ){
	  int k;
          Bool found = FALSE;
            a->pref_preconds = ( int * ) calloc( no->num_pref_preconds, sizeof( int ) );
      a->num_pref_preconds = 0;
          for ( i = 0; i < no->num_pref_preconds; i++ ) {
	      found = FALSE;
	    lp = no->pref_preconds[i].predicate;
	    for ( j = 0; j < garity[lp]; j++ ) {
	        largs[j] = ( no->pref_preconds[i].args[j] >= 0 ) ?
	        no->pref_preconds[i].args[j] : a->inst_table[DECODE_VAR( no->pref_preconds[i].args[j] )];
	    }
	    adr = fact_adress();	
	    ...
	    */
	    if ( !lneg[lp][adr] ) {
	      continue;
                 ...
                 a->pref_preconds[a->num_pref_preconds++] = lindex[lp][adr];
                 for(k = 0; k < gnum_pref_preconds_predicate; k++ ){
	    	   if(relevant_pref_preconds_predicate[k] == lindex[lp][adr]){
		       found = TRUE;
                       break;         
		     } 
		 }
		 if(!found)
                     relevant_pref_preconds_predicate[gnum_pref_preconds_predicate++] = lindex[lp][adr];
            }
        }
      /* end of pref preconds */
	...
	*/
	if ( !lneg[lp][adr] ) {
	  continue;
        ...
        if(isDurative)
              a->preconds_t[a->num_preconds] = no->preconds[i].predicate_t;
...
/*----  for time windows literals  ---*/
    ...
    if(isTempTimeWindows){
           if ( no->ttw_num_preconds > 0 ) {
	
             a->time_windows_list = new_TimeWindows();
             a->array_interval_list = ( TimeWindows_pointer * ) calloc( no->ttw_num_preconds, sizeof( TimeWindows_pointer ) );

              a->tw_pre = ( TokenList_pointer * ) calloc(no->ttw_num_preconds, sizeof( TokenList_pointer ) );
              a->tw_pre_t = ( int * ) calloc( no->ttw_num_preconds, sizeof( int ) );

     /*--- make objects of arguments ttw_preconds and search ---*/
     
              for(i = 0; i < no->ttw_num_preconds; i++){
      
		
                     f = &(no->ttw_preconds[i]);
                     head = NULL;
                     fact_element = ( TokenList * ) calloc( 1, sizeof(TokenList ) );
                     fact_element->item = gpredicates[f->predicate];
		...
		for(j = 0; j < garity[f->predicate];j++ ){
			...
			fact_element->item = gconstants[(f->args)[j]];

			}

		      else {
                       
			  fact_element->item = gconstants[a->inst_table[DECODE_VAR(f->args[j])]];

			  }

		  
                    current->next = fact_element;
                    fact_element->next = NULL;

		}
             
                a->tw_pre[i] = head;
                a->tw_pre_t[i] = no->ttw_preconds[i].predicate_t;
           
                if(k>0){

		    for(l = 0; l < k; l++){
                        if(!same_TokenList(twpredicate[l], head))
                            continue;
                        else
			    break;
		    }
        ...
        /*--------Replace Time windows Literals in Action a ----*/ 
	      
       
        Time_Ini_Literal *resultpre = NULL;
	resultpre = SearchPinListPre(head);
        
	if(resultpre!=NULL)
	    a->array_interval_list[i] = resultpre->tw;
	   
    /*--End of Replace Time windows Literals in Action a ----*/
         
      
   }
                
  /*---end of make objects of arguments ttw_preconds and search ----*/


/*------ begin of calculation of intersection of intervals ---*/

    time_windows_help = a->array_interval_list[0];

    for(i = 1; i < no->ttw_num_preconds; i++){
       
	time_windows_help = calculte_inter_between_Two_interval_list(time_windows_help, a->array_interval_list[i]);

    }

    a->time_windows_list = time_windows_help;   

    /* when time_windows_list empty
     */ 
    if(a->time_windows_list==NULL){
       
        a->time_windows_list = new_TimeWindows();
	a->time_windows_list->mintime = a->timeMin;
        a->time_windows_list->maxtime = a->timeMax;
        time_windows_help = a->time_windows_list;       
    }  

/*------ end of calculation of intersection of intervals -----*/

    num_interv = 0;
    /*  printf(" windows show: \n");*/
    
	while(time_windows_help!=NULL){
            ++num_interv;
	    /*  printf("\n[min, max]: [%.2f, %.2f]\n", time_windows_help->mintime,time_windows_help->maxtime);*/
            time_windows_help = time_windows_help->next;
	}
        a->num_interval = num_interv;  
	/*  printf("\n windows number: %i\n", a->num_interval);*/

      }

       else /* for action that have no time windows condition */
       {
           a->num_interval = 1;
           a->time_windows_list = new_TimeWindows();
	   a->time_windows_list->mintime = a->timeMin;
...
/*----end for time windows literals ----*/
	...
	a->numeric_preconds_lh = ( ExpNode_pointer * )
     ...
     if(isDurative==TRUE)
	a->numeric_preconds_lh_t = ( int * )
	calloc( no->num_numeric_preconds, sizeof( int ) );
	...
	a->num_numeric_preconds = 0;
      ...
      for ( i = 0; i < no->num_numeric_preconds; i++ ) {
        ...
        if(isDurative==TRUE)
	a->numeric_preconds_lh_t[a->num_numeric_preconds]= no->numeric_preconds_lh[i]->lh_t; 
	...
	if ( ne->num_conditions > 0 ) {
	  aa->conditions = ( int * ) calloc( ne->num_conditions, sizeof( int ) );
          if(isDurative)
             aa->conditions_t = ( int * ) calloc( ne->num_conditions, sizeof( int ) );
	...
	if(isDurative)
         aa->conditions_t[aa->num_conditions-1] = ne->conditions[i].predicate_t ;
	...
	if ( ne->num_numeric_conditions > 0 ) {
	  ...
	  aa->numeric_conditions_lh = ( ExpNode_pointer * )
	    calloc( ne->num_numeric_conditions, sizeof( ExpNode_pointer ) );
          ...
          if(isDurative)
               aa->numeric_conditions_lh_t = ( int * )
	    calloc( ne->num_numeric_conditions, sizeof( int ) );
	...
	for ( i = 0; i < ne->num_numeric_conditions; i++ ) {
	 ...
	 if(isDurative)
           aa->numeric_conditions_lh_t[aa->num_numeric_conditions] = ne->numeric_conditions_lh[i]->lh_t;
	...
	if ( ne->num_adds > 0 ) {
	  aa->adds = ( int * ) calloc( ne->num_adds, sizeof( int ) );
          ...
          if(isDurative)
	     aa->adds_t = ( int * ) calloc( ne->num_adds, sizeof( int ) );
	...
	for ( i = 0; i < ne->num_adds; i++ ) {
	  ...
	  if(isDurative)
	  aa->adds_t[aa->num_adds-1] =  ne->adds[i].prednorm_t;
	...
	if ( ne->num_dels > 0 ) {
	  aa->dels = ( int * ) calloc( ne->num_dels, sizeof( int ) );
          ...
          if(isDurative)
	     aa->dels_t = ( int * ) calloc( ne->num_dels, sizeof( int ) );
	...
	for ( i = 0; i < ne->num_dels; i++ ) {
	  ...
	  if(isDurative)
          aa->dels_t[aa->num_dels-1] =  ne->dels[i].prednorm_t;
	...
	for ( i = 0; i < ne->num_numeric_effects; i++ ) {
	  ...
	  if(isDurative)
	  aa->numeric_effects_fl_t[i] = ne->numeric_effects_fluent[i].function_t;
	...
	if(isDurative==TRUE){
            ...
            a->action_duration = ( ExpNode * )
	  calloc( 1, sizeof( ExpNode ) );
	   a->action_duration = copy_Exp(pa->pseudo_duration) ;
	   instantiate_exp_by_action( &(a->action_duration), a );
           set_relevants_in_exp(&(a->action_duration));
          }
	...
	a->preconds = ( int * ) calloc( pa->num_preconds, sizeof( int ) );
        ...
        if(isDurative)
          a->preconds_t = ( int * ) calloc( pa->num_preconds, sizeof( int ) );
      }
      a->num_preconds = 0;
	...
	if(isDurative)
        a->preconds_t[a->num_preconds-1] = pa->preconds[i].predicate_t ;
      }
...
/*----  for time windows literals  ---*/
    ...
    if(isTempTimeWindows){
           if ( pa->ttw_num_preconds > 0 ) {
	
             a->time_windows_list = new_TimeWindows();
             a->array_interval_list = ( TimeWindows_pointer * ) calloc( pa->ttw_num_preconds, sizeof( TimeWindows_pointer ) );

              a->tw_pre = ( TokenList_pointer * ) calloc(pa->ttw_num_preconds, sizeof( TokenList_pointer ) );
              a->tw_pre_t = ( int * ) calloc( pa->ttw_num_preconds, sizeof( int ) );

     /*--- make objects of arguments ttw_preconds and search ---*/
     
              for(i = 0; i < pa->ttw_num_preconds; i++){
		...
		for(j = 0; j < garity[f->predicate];j++ ){
			...
			fact_element->item = gconstants[(f->args)[j]];
			  ...
			  fact_element->item = gconstants[a->inst_table[DECODE_VAR(f->args[j])]];
                ...
                a->tw_pre[i] = head;
                a->tw_pre_t[i] = pa->ttw_preconds[i].predicate_t;
...
/*------ begin of calculation of intersection of intervals ---*/
    ...
    for(i = 1; i < pa->ttw_num_preconds; i++){
...
/*------ end of calculation of intersection of intervals -----*/
	...
	/*  printf("\n windows number: %i\n", a->num_interval);*/
       ...
       else /* for action that have pa time windows condition */
       {
           a->num_interval = 1;
           a->time_windows_list = new_TimeWindows();
	   a->time_windows_list->mintime = a->timeMin;
...
/*----end for time windows literals ----*/
	...
	if(isDurative)
           	a->numeric_preconds_lh_t = ( int * )
	  calloc( pa->num_numeric_preconds, sizeof( int ) );
	...
	a->num_numeric_preconds = 0;
      ...
      for ( i = 0; i < pa->num_numeric_preconds; i++ ) {
       ...
       if(isDurative)
        a->numeric_preconds_lh_t[a->num_numeric_preconds] = pa->numeric_preconds_lh[i]->lh_t;
	...
	if ( pae->num_conditions > 0 ) {
	  ...
	  if(isDurative)
             aa->conditions_t = ( int * ) calloc( pae->num_conditions, sizeof( int ) );
	...
	for ( i = 0; i < pae->num_conditions; i++ ) {
	  ...
	  if(isDurative)
          aa->conditions_t[aa->num_conditions-1] = pae->conditions[i].predicate_t ;
	}
	...
	if ( pae->num_numeric_conditions > 0 ) {
	  ...
	  aa->numeric_conditions_lh = ( ExpNode_pointer * )
	    calloc( pae->num_numeric_conditions, sizeof( ExpNode_pointer ) );
          ...
          if(isDurative)
              aa->numeric_conditions_lh_t = ( int * )
	    calloc( pae->num_numeric_conditions, sizeof( int ) );
	...
	for ( i = 0; i < pae->num_numeric_conditions; i++ ) {
	    ...
	    if(isDurative)
           aa->numeric_conditions_lh_t[aa->num_numeric_conditions] = pae->numeric_conditions_lh[i]->lh_t;
	...
	if ( pae->num_adds > 0 ) {
	  aa->adds = ( int * ) calloc( pae->num_adds, sizeof( int ) );
          ...
          if(isDurative)
            aa->adds_t = ( int * ) calloc( pae->num_adds, sizeof( int ) );
	}
	aa->num_adds = 0;
	for ( i = 0; i < pae->num_adds; i++ ) {
	  ...
	  if(isDurative)
          aa->adds_t[aa->num_adds-1] = pae->adds[i].prednorm_t ;
	...
	if ( pae->num_dels > 0 ) {
	  aa->dels = ( int * ) calloc( pae->num_dels, sizeof( int ) );
          ...
          if(isDurative)
             aa->dels_t = ( int * ) calloc( pae->num_dels, sizeof( int ) );
	...
	for ( i = 0; i < pae->num_dels; i++ ) {
	  ...
	  aa->dels[aa->num_dels++] = lindex[lp][adr];
          ...
          if(isDurative)
          aa->dels_t[aa->num_dels-1] = pae->dels[i].prednorm_t ;
	...
	for ( i = 0; i < pae->num_numeric_effects; i++ ) {
	  ...
	  if(isDurative)
          aa->numeric_effects_fl_t[i] = pae->numeric_effects_fluent[i].function_t;
...

{
  ...
  for ( a = gactions; a; a = a->next ) {
   ...
   calloc( e->num_conditions + a->num_preconds, sizeof( int ) );
	...
	/* if(isDurative)	
	   gef_conn[n_ef].PC_t[gef_conn[n_ef].num_PC-1] = a->preconds_t[j];*/
      }
	...
	gef_conn[n_ef].PC[gef_conn[n_ef].num_PC++] = e->conditions[j];
         ...
         if(isDurative)	
               gef_conn[n_ef].PC_t[gef_conn[n_ef].num_PC-1] = e->conditions_t[j];
	...
	if ( k < gef_conn[n_ef].num_f_PC ) {
	  ...
	  if(isDurative)
	      gef_conn[n_ef].f_PC_fl_t[gef_conn[n_ef].num_f_PC] = a->lnf_preconds_lh_t[j];
	...
	if ( k < gef_conn[n_ef].num_f_PC ) {
	  ...
	  gef_conn[n_ef].f_PC_fl[gef_conn[n_ef].num_f_PC] = e->lnf_conditions_lh[j]->pF[0];
	   ...
	   if(isDurative)
	         gef_conn[n_ef].f_PC_fl_t[gef_conn[n_ef].num_f_PC] = e->lnf_conditions_lh_t[j];
	...
	continue;
      ...
      /*  if(isDurative)
	  gef_conn[n_ef].A_t = ( int * ) calloc( e->num_adds, sizeof( int ) );*/

      gef_conn[n_ef].D = ( int * ) calloc( e->num_dels, sizeof( int ) );

      /* if(isDurative)
	 gef_conn[n_ef].D_t = ( int * ) calloc( e->num_dels, sizeof( int ) );*/

      gef_conn[n_ef].IN_fl = ( int * ) calloc( e->num_lnf_effects, sizeof( int ) );

      if(isDurative)
           gef_conn[n_ef].IN_fl_t = ( int * ) calloc( e->num_lnf_effects, sizeof( int ) );
      ...
      if(isDurative)
         gef_conn[n_ef].AS_fl_t = ( int * ) calloc( e->num_lnf_effects, sizeof( int ) );
	...
	/* if(isDurative)       
	   gef_conn[n_ef].A_t[gef_conn[n_ef].num_A-1] = e->adds_t[j];*/
	...
	/* if(isDurative)
	   gef_conn[n_ef].D_t[gef_conn[n_ef].num_D-1] = e->dels_t[j];*/
	...
	if(isDurative)
            gef_conn[n_ef].IN_fl_t[gef_conn[n_ef].num_IN] = e->lnf_effects_fl_t[j];
	...
	if(isDurative)
               gef_conn[n_ef].AS_fl_t[gef_conn[n_ef].num_AS] = e->lnf_effects_fl_t[j];
