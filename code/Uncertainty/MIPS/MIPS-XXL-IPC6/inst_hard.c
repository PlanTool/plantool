...

{
  ...
  case OR:
	...
	for ( ww = w->sons; ww; ww = ww->next ) {
	  if ( ww->connective == ATOM ) {
	    tmp2->preconds[m].predicate = ww->fact->predicate;
            ...
            tmp2->preconds[m].predicate_t = ww->fact->predicate_t;
	  ...
	  if ( ww->connective == COMP ) {
	    ...
	    tmp2->numeric_preconds_lh[mn] = copy_Exp( ww->lh );
            ...
            tmp2->numeric_preconds_lh[mn]->lh_t = ww->lh->lh_t;   
	...
	if ( w->connective == ATOM ) {
	  ...
	  tmp2->preconds[0].predicate = w->fact->predicate;
          ...
          tmp2->preconds[0].predicate_t = w->fact->predicate_t;          
	...
	if ( w->connective == COMP ) {
	  ...
	  tmp2->numeric_preconds_lh[0] = copy_Exp( w->lh );
          ...
          tmp2->numeric_preconds_lh[0]->lh_t = w->lh->lh_t;
      ...
      tmp2->effects = instantiate_Effect( o->effects );
       ...
       if(isDurative==TRUE)
             tmp2->operator_duration = o->operator_duration;

      /* for timed initial literal preconds
      **/
      if(isTempTimeWindows)
           make_mixed_ttw_preconds(tmp2, o);
  ...
  case AND:
	...
	tmp2->preconds[m].predicate = w->fact->predicate;
        ...
        tmp2->preconds[m].predicate_t = w->fact->predicate_t;
	...
	tmp2->numeric_preconds_lh[mn] = copy_Exp( w->lh );
        ...
        tmp2->numeric_preconds_lh[mn]->lh_t = w->lh->lh_t;
  ...
  case ATOM:
    ...
    tmp2->preconds[0].predicate_t = tmp1->fact->predicate_t;
  ...
  case COMP:
    ...
    tmp2->numeric_preconds_lh[0]->lh_t = tmp1->lh->lh_t;
...
/*--- for make ttw_preconds in MixedOperator----------*/



void make_mixed_ttw_preconds( MixedOperator *mop, Operator *op )
...
switch ( z->connective ) {
	...
	if ( zz->connective == AND ) {
	  ...
	  mop->ttw_preconds = ( Fact * ) calloc( m, sizeof( Fact ) );
	  
	  for ( zzz = zz->sons; zzz; zzz = zzz->next ) {
	    if ( zzz->connective == ATOM ) {
	      tmp_ft_ttw = &(mop->ttw_preconds[mop->ttw_num_preconds]);
...
/*-------end----temporale--Daten--------------*/
	      ...
	      mop->ttw_num_preconds++;
	    }
	    
	  }
	} else {
	  if ( zz->connective == ATOM ) {
	    mop->ttw_preconds = ( Fact * ) calloc( 1, sizeof( Fact ) );
	    tmp_ft_ttw = &(mop->ttw_preconds[0]);
...
/*---end  make ttw_preconds in MixedOperator--------*/
...

{
  ...
  for ( i = e; i; i = i->next ) {
    ...
    for ( ne = i->numeric_effects; ne; ne = ne->next ) {
      ...
      ttt->fluent.function_t = ne->fluent_t;
...

{
  ...
  case AND:
    ...
    while ( i ) {
      ...
      /* give time_info to instantiate ob
      **/
      if(isDurative){
        if(i->fact_t != 0)
          tmp->fact_t = i->fact_t;
  ...
  case COMP:
    ...
    /* give time info to instantiate ob
    **/
    if(isDurative){
      if(w->lh->lh_t != 0)
	res->lh->lh_t = w->lh->lh_t;
    }
...

{
  ...
  for ( o = ghard_mixed_operators; o; o = o->next ) {
    ...
    if(isDurative==TRUE)
	tmp->pseudo_duration = o->operator_duration;
...

{
  ...
  case OR:
	...
	for ( ww = w->sons; ww; ww = ww->next ) {
	  if ( ww->connective == ATOM ) {
	    tmp2->conditions[m].predicate = ww->fact->predicate;
           ...
           tmp2->conditions[m].predicate_t = ww->fact_t;
	  ...
	  if ( ww->connective == COMP ) {
	    ...
	    tmp2->numeric_conditions_lh[mn] = copy_Exp( ww->lh );
             ...
             tmp2->numeric_conditions_lh[mn]->lh_t = ww->lh->lh_t;
	...
	if ( w->connective == ATOM ) {
	  ...
	  tmp2->conditions[0].predicate = w->fact->predicate;
          ...
          tmp2->conditions[0].predicate_t = w->fact_t;
 	...
 	if ( w->connective == COMP ) {
	  ...
	  tmp2->numeric_conditions_lh[0] = copy_Exp( w->lh );
          ...
          tmp2->numeric_conditions_lh[0]->lh_t =  w->lh->lh_t;
  ...
  case AND:
	...
	tmp2->conditions[m].predicate = w->fact->predicate;
        ...
        tmp2->conditions[m].predicate_t = w->fact_t;
	...
	tmp2->numeric_conditions_lh[mn] = copy_Exp( w->lh );
        ...
        tmp2->numeric_conditions_lh[mn]->lh_t = w->lh->lh_t;
  ...
  case ATOM:
    ...
    tmp2->conditions[0].predicate_t = tmp1->fact_t;
  ...
  case COMP:
    ...
    tmp2->numeric_conditions_lh[0]->lh_t = tmp1->lh->lh_t;
...

{
  ...
  for ( l = ll; l; l = l->next ) {
    if ( l->negated ) {
      ...
      e->dels[e->num_dels].prednorm_t = l->fact.prednorm_t;
	...
	e->dels[e->num_dels].args[i] = ( l->fact.args[i] < 0 ) ?
      ...
      e->adds[e->num_adds].prednorm_t = l->fact.prednorm_t;
...

{
  ...
  for ( n = ne; n; n = n->next ) {
    ...
    e->numeric_effects_fluent[m].function_t = n->fluent.function_t;
