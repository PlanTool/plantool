...

{
  ...
  for ( f = gfunctions_and_types; f; f = f->next ) {
    ...
    for ( t = f->item->next; t; t = t->next ) {
      if ( (type_num = position_in_types_table( t->item )) == -1 ) {
	printf("\nWarning! Function %s is declared to use unknown or empty type %s\n\n", 
	       f->item->item, t->item);
	/* exit( 1 );*/
      }
      if ( ar == MAX_ARITY ) {
...

{
  ...
  **/
  if(isPreference){
      if(pref_head)
           gpref_head = make_Wff( pref_head, 0 );   
  }
  ...
  **/
  if(isConstraints){
      if(gorig_constraints_facts)
           gconstraints = make_Wff( gorig_constraints_facts, 0 );
      if(til_constraints)
           gtil_constraints = make_Wff( til_constraints, 0 );

      if(within_constraints)
           gwithin_constraints = make_Wff( within_constraints, 0 );
    
    if(isConstraintsPreference){
      if(pref_constraints_head)
         gconstraints_pref = make_Wff(pref_constraints_head , 0 );
      if(til_constraints_pref)
         gtil_constraints_pref = make_Wff( til_constraints_pref, 0 );

      if(within_constraints_pref)
         gwithin_constraints_pref = make_Wff( within_constraints_pref, 0 );
  ...
  for ( o = gloaded_ops; o; o = o->next ) {
    ...
    /* for preference precondition
    **/
    if(isDomainPreference){
          mark_preference_in_goal(o->preconds);
          tmp->pref_preconds = make_Wff(extract_preference_from_goal(o->preconds), tmp->num_vars );
     ...
     if(isTempTimeWindows)
      tmp->ttw_preconds = make_Wff(o->ttw_preconds, tmp->num_vars);

    if ( o->effects ) {
      /* in make_effect, make sure that no one afects equality.
       */

         /* when effects have one effect, then assign his temp to this effect
          */
	if(isDurative==TRUE)
         if(o->effects->sons->sons->next->sons->next==NULL){

	  if(o->effects->atom_t!=0)
	       o->effects->sons->sons->next->sons->atom_t = o->effects->atom_t;
          }
    ...
    if(isDurative==TRUE)
    tmp->operator_duration =make_ExpNode(o->duration,tmp->num_vars);
...

{
  ...
  if(isConstraintsPreference || isPreference)
      if(p->name){
  ...
  case COMP:
   ...
   case TEMPORALOP:
     ...
     tmp->son = make_Wff( p->sons, num_vars );
     if((p->temporal_op==SOMETIME_AFTER) || (p->temporal_op==SOMETIME_BEFORE) || (p->temporal_op==ALWAYS_WITHIN))
      tmp->son->next = make_Wff( p->sons->next, num_vars );
    break;
  default:
...

{
  ...
  for ( m = n->sons->next->sons; m; m = m->next ) {
	...
	tmp->numeric_effects->rh = make_ExpNode( m->rh, num_vars + tmp->num_vars );
      ...
      tmp->numeric_effects->fluent_t = m->atom_t;
...

{
  ...
  /* for not problem with preferences
  **/
  /* if((isPreference == FALSE) || ((isPreference == TRUE) && ( ggoal->connective != TRU )))*/
  ...
  **/
  if(isPreference) {
    if(gpref_head)  
            expand_quantifiers_in_wff( &gpref_head, -1, -1 );

    if(gpref_head)
    if ( gpref_head->connective == TRU ) {
      gpref_head = NULL;
    printf("\nff: gpref_head can be simplified to TRUE. The empty plan satisfied it\n\n");
  }
  if(gpref_head)
      if ( gpref_head->connective == FAL ) {
    printf("\nff: gpref_head can be simplified to FALSE. No plan will satisfy it\n\n");
  }

  /*for clean up gpref_head
  **/
  if(gpref_head){
     preference_simple_node(gpref_head,pref_head );
    WffNode *i_son;
    cleanup_wff( &(gpref_head->sons) );

    for(i_son = gpref_head->sons->next; i_son!=NULL; i_son = i_son->next ){
        cleanup_wff(&(i_son));
       }

      cleanup_all_wff_pref(gpref_head);
  ...
  **/
  if(isConstraints) {
    if(gconstraints)  
            expand_quantifiers_in_wff( &gconstraints, -1, -1 );
    if(gtil_constraints)
            expand_quantifiers_in_wff( &gtil_constraints, -1, -1 );
    if(gwithin_constraints)
            expand_quantifiers_in_wff( &gwithin_constraints, -1, -1 );
    if(isConstraintsPreference){
       if(gconstraints_pref)
            expand_quantifiers_in_wff ( &gconstraints_pref, -1, -1 );
       if(gtil_constraints_pref)
            expand_quantifiers_in_wff( &gtil_constraints_pref, -1, -1 );
       if(gwithin_constraints_pref){       
            expand_quantifiers_in_wff( &gwithin_constraints_pref, -1, -1 );
       }
    ...
    if(gconstraints){
/*
		if (gconstraints->sons == NULL){
			printf("gconstraints->sons are empty!!!!!\n");
			
		}else{
			printf("gconstraints->sons not empty!!! \n");
		}
*/
       if ( gconstraints->connective == TRU ) {
		   gconstraints = NULL;
		   printf("\nff: gconstraints can be simplified to TRUE. The empty plan satisfied it\n\n");
	   }
	}
	if(gconstraints)
		if ( gconstraints->connective == FAL ) {
			printf("\nff: gconstraints can be simplified to FALSE. No plan will satisfy it\n\n");
			exit( 1 );
		}


  if(gconstraints_pref)
    if ( gconstraints_pref->connective == TRU ) {
      gconstraints_pref = NULL;
    printf("\nff: gconstraints_pref can be simplified to TRUE. The empty plan satisfied it\n\n");
  }
  if(gconstraints_pref)
      if ( gconstraints_pref->connective == FAL ) {
    printf("\nff: gconstraints_pref can be simplified to FALSE. No plan will satisfy it\n\n");
  }


  if(gtil_constraints)
       if ( gtil_constraints->connective == TRU ) {
      gtil_constraints = NULL;
    printf("\nff: gtil_constraints can be simplified to TRUE. The empty plan satisfied it\n\n");
  }
  if(gtil_constraints)
      if ( gtil_constraints->connective == FAL ) {
    printf("\nff: gtil_constraints can be simplified to FALSE. No plan will satisfy it\n\n");
    exit( 1 );
  }

   if(gwithin_constraints)
       if ( gwithin_constraints->connective == TRU ) {
      gwithin_constraints = NULL;
    printf("\nff: gwithin_constraints can be simplified to TRUE. The empty plan satisfied it\n\n");
  }
  if(gwithin_constraints)
      if ( gwithin_constraints->connective == FAL ) {
    printf("\nff: gwithin_constraints can be simplified to FALSE. No plan will satisfy it\n\n");
    exit( 1 );
  }


 if(gtil_constraints_pref)
       if ( gtil_constraints_pref->connective == TRU ) {
      gtil_constraints_pref = NULL;
    printf("\nff: gtil_constraints_pref can be simplified to TRUE. The empty plan satisfied it\n\n");
  }
  if(gtil_constraints_pref)
      if ( gtil_constraints_pref->connective == FAL ) {
    printf("\nff: gtil_constraints_pref can be simplified to FALSE. No plan will satisfy it\n\n");
    exit( 1 );
  }

    if(gwithin_constraints_pref)
       if ( gwithin_constraints_pref->connective == TRU ) {
      gwithin_constraints_pref = NULL;
    printf("\nff: gwithin_constraints_pref can be simplified to TRUE. The empty plan satisfied it\n\n");
  }
  if(gwithin_constraints_pref)
      if ( gwithin_constraints_pref->connective == FAL ) {
    printf("\nff: gwithin_constraints_pref can be simplified to FALSE. No plan will satisfy it\n\n");
    exit( 1 );
  }

  /*for clean up gconstraints_pref
  **/
  if(gconstraints_pref){

    preference_simple_node(gconstraints_pref,pref_constraints_head);

    WffNode *i_son;
    if((gconstraints_pref->sons->temporal_op == SOMETIME_BEFORE) || (gconstraints_pref->sons->temporal_op == SOMETIME_AFTER)){
          cleanup_wff( &(gconstraints_pref->sons->son) );
          cleanup_wff( &(gconstraints_pref->sons->son->next) );
    }
    else
        cleanup_wff( &(gconstraints_pref->sons) );


    for(i_son = gconstraints_pref->sons->next; i_son!=NULL; i_son = i_son->next ){
      if((i_son->temporal_op == SOMETIME_BEFORE) || (i_son->temporal_op == SOMETIME_AFTER)){
          cleanup_wff(&(i_son->son));
          cleanup_wff(&(i_son->son->next));
       }
      else
        cleanup_wff(&(i_son));
    }

     cleanup_all_wff_pref(gconstraints_pref);

   }

    /*for clean up gtil_constraints_pref
    **/
  if(gtil_constraints_pref){
    preference_simple_node(gtil_constraints_pref,til_constraints_pref);
    WffNode *i_son;
    cleanup_wff( &(gtil_constraints_pref->sons) );

    for(i_son = gtil_constraints_pref->sons->next; i_son!=NULL; i_son = i_son->next ){
        cleanup_wff(&(i_son));
       }

         cleanup_all_wff_pref(gtil_constraints_pref);

   }

   /*for clean up gwithin_constraints_pref
    **/
  if(gwithin_constraints_pref){

    preference_simple_node(gwithin_constraints_pref,within_constraints_pref);
  
    WffNode *i_son;
    if(gwithin_constraints_pref->sons->temporal_op == ALWAYS_WITHIN){
          cleanup_wff( &(gwithin_constraints_pref->sons->son) );
          cleanup_wff( &(gwithin_constraints_pref->sons->son->next) );
    }
    else
       cleanup_wff( &(gwithin_constraints_pref->sons) );

    for(i_son = gwithin_constraints_pref->sons->next; i_son!=NULL; i_son = i_son->next ){
         if(i_son->temporal_op == ALWAYS_WITHIN){
          cleanup_wff(&(i_son->son));
          cleanup_wff(&(i_son->son->next));
         }
         else
        ...
        cleanup_all_wff_pref(gwithin_constraints_pref);
        
        
   }
    

    /*for clean up gconstraints
    **/



/*	if (gconstraints->sons == NULL){
			printf("gconstraints->sons are still empty!!!!!\n");
			gconstraints = NULL;
	}
*/

  /*
   * Fixes done on Aug. 17, 2007. 
   */
   if(gconstraints){
	   /*	printf("************in the IF CHECK\n");*/
    WffNode *i_son;
     if((gconstraints->sons->temporal_op == SOMETIME_BEFORE) || (gconstraints->sons->temporal_op == SOMETIME_AFTER)){
          cleanup_wff( &(gconstraints->sons->son) );
          cleanup_wff( &(gconstraints->sons->son->next) );
    }
    else
        cleanup_wff( &(gconstraints->sons) );

	 
    for(i_son = gconstraints->sons->next; i_son!=NULL; i_son = i_son->next ){
	...
	/*printf("************CLEANING UP ALL WFF\n");*/
    cleanup_all_wff(gconstraints);

	if (gconstraints->sons == NULL){
		/*	printf("gconstraints->sons are now empty!!!!!\n"); */
			gconstraints = NULL;
	}
	else
		splitt_ltl_formel_for_constraints(gconstraints, gorig_constraints_facts);

   }


    /*for clean up gtil_constraints
    **/
   if(gtil_constraints){

    WffNode *i_son;
    cleanup_wff( &(gtil_constraints->sons) );

    for(i_son = gtil_constraints->sons->next; i_son!=NULL; i_son = i_son->next ){
        cleanup_wff(&(i_son));
       }
     cleanup_all_wff(gtil_constraints);

   }
   
    /*for clean up gwithin_constraints
    **/
   if(gwithin_constraints){

    WffNode *i_son;
     if(gwithin_constraints->sons->temporal_op == ALWAYS_WITHIN){
          cleanup_wff( &(gwithin_constraints->sons->son) );
          cleanup_wff( &(gwithin_constraints->sons->son->next) );
    }
    else
        cleanup_wff( &(gwithin_constraints->sons) );

    for(i_son = gwithin_constraints->sons->next; i_son!=NULL; i_son = i_son->next ){
         if(i_son->temporal_op == ALWAYS_WITHIN){
          cleanup_wff(&(i_son->son));
          cleanup_wff(&(i_son->son->next));
         }
      ...
      cleanup_all_wff(gwithin_constraints);
   }
  ...
  for ( i = 0; i < gnum_operators; i++ ) {
    ...
    **/
    if(goperators[i]->pref_preconds){
       simplify_wff( &(goperators[i]->pref_preconds) );
       remove_unused_vars_in_wff( &(goperators[i]->pref_preconds) );
       expand_quantifiers_in_wff( &(goperators[i]->pref_preconds), -1, -1 );
       NOTs_down_in_wff( &(goperators[i]->pref_preconds) );
       /*cleanup_wff( &(goperators[i]->pref_preconds) );*/

       if(goperators[i]->pref_preconds)
          if ( goperators[i]->pref_preconds->connective == TRU ) {
               goperators[i]->pref_preconds = NULL;
       printf("\nff: goperators[%d]->pref_preconds can be simplified to TRUE. The empty plan satisfied it\n\n", i);
    }
     if(goperators[i]->pref_preconds)
         if ( goperators[i]->pref_preconds->connective == FAL ) {
       printf("\nff: goperators[%d]->pref_preconds can be simplified to FALSE. No plan will satisfy it\n\n", i);
      exit( 1 );
     }
    }
    ...
    **/
    if(isDurative==TRUE)
     simplify_exp( &(goperators[i]->operator_duration) );
...

{
  ...
  case TEMPORALOP:
    remove_unused_vars_in_wff( &((*w)->son) );
     if(((*w)->temporal_op==SOMETIME_AFTER) || ((*w)->temporal_op==SOMETIME_BEFORE) || ((*w)->temporal_op==ALWAYS_WITHIN))
       remove_unused_vars_in_wff( &((*w)->son->next) );
    break;
  case COMP:
  case ATOM:
  case TRU:
...

{
  ...
  case COMP:
   ...
   case TEMPORALOP:
       ...
       /* if( var_used_in_wff( code_var, w->son )){
              return TRUE;
       }

       if(w->son->son){
	       if(var_used_in_wff( code_var, w->son->son )){
               return TRUE;
	       }
	       }*/
...

{
  ...
  case TEMPORALOP:
    ...
    decrement_inferior_vars( var, w->son );
     if((w->temporal_op==SOMETIME_AFTER) || (w->temporal_op==SOMETIME_BEFORE) || (w->temporal_op==ALWAYS_WITHIN))
          decrement_inferior_vars( var, w->son->next );
    break;
  case TRU:
  case FAL:
    break;
...

{
  ...
  case AND:
    ...
    if ( m == 1 ) {
      ...
      /* for temporal operator in preference
      **/
      if((*w)->sons->connective == TEMPORALOP)
	 (*w)->temporal_op = (*w)->sons->temporal_op;
  
      /* for preference name in preconds with one sons
      **/
      if((*w)->sons->name)
        (*w)->name = (*w)->sons->name;
  ...
  case TEMPORALOP:
      ...
      simplify_wff( &((*w)->son) );
       if(((*w)->temporal_op==SOMETIME_AFTER) || ((*w)->temporal_op==SOMETIME_BEFORE) || ((*w)->temporal_op==ALWAYS_WITHIN))
         simplify_wff( &((*w)->son->next) );

    if ( (*w)->son->connective == TRU ||
	 (*w)->son->connective == FAL ) {
      (*w)->connective = ( (*w)->son->connective == TRU ) ? TRU : FAL;
      free( (*w)->son );
      (*w)->son = NULL;
    }
      break;
...

{
  ...
  case EX:
    ...
    for ( j = 0; j < gtype_size[(*w)->var_type]; j++ ) {
      ...
      /* give time_info to son
      **/
      if((*w)->fact_t)
          tmp->fact_t = (*w)->fact_t;
      
      /*for constraints preference and preferences
      **/
       if((*w)->son->name){

	   sprintf(s, "%s_%d", (*w)->son->name, j);
           tmp->name = ( char * ) calloc( 20, sizeof( char ) );
	     tmp->new_name = ( char * ) calloc( 20, sizeof( char ) );
           strcpy(tmp->name, s);
           strcpy(tmp->new_name, s);

	   /* for son->son
	   **/
           if(((*w)->son->connective == ALL) ||((*w)->son->connective == EX) ){
	     if((*w)->son->son)
	       if((*w)->son->son->name){
                   tmp->son->name = ( char * ) calloc( 20, sizeof( char ) );
	           tmp->son->new_name = ( char * ) calloc( 20, sizeof( char ) );
                   strcpy(tmp->son->name, s);
                   strcpy(tmp->son->new_name, s);
		   
	       }
                /* for son->son->son
	        **/
	       if((*w)->son->son){
                   if(((*w)->son->son->connective == ALL) ||((*w)->son->son->connective == EX) ){
		  if((*w)->son->son->son)
	             if((*w)->son->son->son->name){
                        tmp->son->son->name = ( char * ) calloc( 20, sizeof( char ) );
	                tmp->son->son->new_name = ( char * ) calloc( 20, sizeof( char ) );
                        strcpy(tmp->son->son->name, s);
                        strcpy(tmp->son->son->new_name, s);
	               ...
	               **/
		   if((*w)->son->son->son){
                           if(((*w)->son->son->son->connective == ALL) ||((*w)->son->son->son->connective == EX) ){
			if((*w)->son->son->son->son)
	                  if((*w)->son->son->son->son->name){
                             tmp->son->son->son->name = ( char * ) calloc( 20, sizeof( char ) );
	                     tmp->son->son->son->new_name = ( char * ) calloc( 20, sizeof( char ) );
                             strcpy(tmp->son->son->son->name, s);
                             strcpy(tmp->son->son->son->new_name, s);
  ...
  case OR:
      for ( i = (*w)->sons; i; i = i->next ) {
       ...
       /* give time_info to sons
       **/
       if(((*w)->fact_t != 0) && (i->fact_t == 0))
           i->fact_t = (*w)->fact_t;
    }
    break;
  case NOT:
  ...
  case TEMPORALOP:
    ...
    expand_quantifiers_in_wff( &((*w)->son), var, constant );
    if(((*w)->temporal_op==SOMETIME_AFTER) || ((*w)->temporal_op==SOMETIME_BEFORE) || ((*w)->temporal_op==ALWAYS_WITHIN))
         expand_quantifiers_in_wff( &((*w)->son->next), var, constant );
    break;
  default:
...

{
  ...
  case TEMPORALOP:
    tmp->temporal_op = w->temporal_op;
     ...
     tmp->son = copy_Wff( w->son );    
      if((w->temporal_op==SOMETIME_AFTER) || (w->temporal_op==SOMETIME_BEFORE) || (w->temporal_op==ALWAYS_WITHIN))
     tmp->son->next = copy_Wff( w->son->next );
    break;
  case TRU:
  case FAL:
    break;
...

{
  ...
  case TEMPORALOP:
    detect_tautologies_in_wff( &((*w)->son) );
     if(((*w)->temporal_op==SOMETIME_AFTER) || ((*w)->temporal_op==SOMETIME_BEFORE) || ((*w)->temporal_op==ALWAYS_WITHIN))
      detect_tautologies_in_wff( &((*w)->son->next) );
    break;
  case ATOM:
  case COMP:
  case TRU:
...

{
  ...
  case TEMPORALOP:
    merge_ANDs_and_ORs_in_wff( &((*w)->son) );
      if(((*w)->temporal_op==SOMETIME_AFTER) || ((*w)->temporal_op==SOMETIME_BEFORE) || ((*w)->temporal_op==ALWAYS_WITHIN))
       merge_ANDs_and_ORs_in_wff( &((*w)->son->next) );
   break;
  case COMP:
  case ATOM:
  case TRU:
...
/*----------temporale--Daten------------------*/
	   ...
	   tmp_op->numeric_preconds_lh[tmp_op->num_numeric_preconds]->lh_t = www->fact_t;
...
/*----------temporale--Daten------------------*/
	    ...
	    tmp_op->numeric_preconds_lh[0]->lh_t = ww->fact_t;
...
/*-------end----temporale--Daten--------------*/
	...
	make_normal_effects( &tmp_op, goperators[i] );
       ...
       tmp_op->normoperator_duration = goperators[i]->operator_duration;

       /* for timed initial literal preconds
       **/
       if(isTempTimeWindows)
          make_normal_ttw_preconds(tmp_op, goperators[i]);

       /* for pref preconds
       **/
       if(goperators[i]->pref_preconds)
          make_normal_pref_preconds(tmp_op, goperators[i]);
...
/*----------temporale--Daten------------------*/
       ...
       tmp_op->numeric_preconds_lh[tmp_op->num_numeric_preconds]->lh_t = ww->fact_t;
...
/*----------temporale--Daten------------------*/
       ...
       tmp_op->numeric_preconds_lh[0]->lh_t = w->fact_t;
...
/*--- for make ttw_preconds in NormOperator----------*/



void make_normal_ttw_preconds( NormOperator *nop, Operator *op )
...
switch ( z->connective ) {
    case OR:
      for ( zz = z->sons; zz; zz = zz->next ) {
     
	if ( zz->connective == AND ) {
	  m = 0;
	  for ( zzz = zz->sons; zzz; zzz = zzz->next ) {
	    if ( zzz->connective == ATOM ) m++;
	    
	  }
	  nop->ttw_preconds = ( Fact * ) calloc( m, sizeof( Fact ) );
	  
	  for ( zzz = zz->sons; zzz; zzz = zzz->next ) {
	    if ( zzz->connective == ATOM ) {
	      tmp_ft_ttw = &(nop->ttw_preconds[nop->ttw_num_preconds]);
	      tmp_ft_ttw->predicate = zzz->fact->predicate;
	      for ( j = 0; j < garity[tmp_ft_ttw->predicate]; j++ ) {
		tmp_ft_ttw->args[j] = zzz->fact->args[j];
	      }

/*----------temporale--Daten------------------*/

	        tmp_ft_ttw->predicate_t = zzz->fact_t;

/*-------end----temporale--Daten--------------*/


	      nop->ttw_num_preconds++;
	    }
	    
	  }
	} else {
	  if ( zz->connective == ATOM ) {
	    nop->ttw_preconds = ( Fact * ) calloc( 1, sizeof( Fact ) );
	    tmp_ft_ttw = &(nop->ttw_preconds[0]);
	    tmp_ft_ttw->predicate = zz->fact->predicate;
	    for ( j = 0; j < garity[tmp_ft_ttw->predicate]; j++ ) {
	      tmp_ft_ttw->args[j] = zz->fact->args[j];
	    }

/*----------temporale--Daten------------------*/

	     tmp_ft_ttw->predicate_t = zz->fact_t;

/*-------end----temporale--Daten--------------*/



	    nop->ttw_num_preconds = 1;
	  }
...
/*-------end----temporale--Daten--------------*/
    ...
    case ATOM:
      ...
      nop->ttw_preconds = ( Fact * ) calloc( 1, sizeof( Fact ) );
      tmp_ft_ttw = &(nop->ttw_preconds[0]);
      tmp_ft_ttw->predicate = z->fact->predicate;
      for ( j = 0; j < garity[tmp_ft_ttw->predicate]; j++ ) {
	tmp_ft_ttw->args[j] = z->fact->args[j];
      }

/*----------temporale--Daten------------------*/

        tmp_ft_ttw->predicate_t = z->fact_t;
...
/*---end  make ttw_preconds in NormOperator--------*/



/*--- for make pref_preconds in NormOperator----------*/



void make_normal_pref_preconds( NormOperator *nop, Operator *op )
...
switch ( z->connective ) {
	...
	if ( zz->connective == AND ) {
	  ...
	  nop->pref_preconds = ( Fact * ) calloc( m, sizeof( Fact ) );
	  
	  for ( zzz = zz->sons; zzz; zzz = zzz->next ) {
	    if ( zzz->connective == ATOM ) {
	      tmp_ft_pref = &(nop->pref_preconds[nop->num_pref_preconds]);
	      tmp_ft_pref->predicate = zzz->fact->predicate;
	      for ( j = 0; j < garity[tmp_ft_pref->predicate]; j++ ) {
		tmp_ft_pref->args[j] = zzz->fact->args[j];
	      }

/*----------temporale--Daten------------------*/

	        tmp_ft_pref->predicate_t = zzz->fact_t;

/*-------end----temporale--Daten--------------*/


	      nop->num_pref_preconds++;
	    }
	    
	  }
	} else {
	  if ( zz->connective == ATOM ) {
	    nop->pref_preconds = ( Fact * ) calloc( 1, sizeof( Fact ) );
	    tmp_ft_pref = &(nop->pref_preconds[0]);
	    tmp_ft_pref->predicate = zz->fact->predicate;
	    for ( j = 0; j < garity[tmp_ft_pref->predicate]; j++ ) {
	      tmp_ft_pref->args[j] = zz->fact->args[j];
	    }

/*----------temporale--Daten------------------*/

	     tmp_ft_pref->predicate_t = zz->fact_t;

/*-------end----temporale--Daten--------------*/



	    nop->num_pref_preconds = 1;
	  }
...
/*-------end----temporale--Daten--------------*/
    ...
    case ATOM:
      ...
      nop->pref_preconds = ( Fact * ) calloc( 1, sizeof( Fact ) );
      tmp_ft_pref = &(nop->pref_preconds[0]);
      tmp_ft_pref->predicate = z->fact->predicate;
      for ( j = 0; j < garity[tmp_ft_pref->predicate]; j++ ) {
	tmp_ft_pref->args[j] = z->fact->args[j];
      }

/*----------temporale--Daten------------------*/

        tmp_ft_pref->predicate_t = z->fact_t;
...
/* edn of pref_preconds  */

 }

/*---end  make pref_preconds in NormOperator--------*/
  ...
  for ( e = op->effects; e; e = e->next ) {
	...
	if ( ww->connective == AND ) {
	  ...
	  for ( www = ww->sons; www; www = www->next ) {
	    ...
	    if ( www->connective == COMP ) {
	      ...
	      tmp_ef->numeric_conditions_lh[tmp_ef->num_numeric_conditions] = copy_Exp( www->lh );
              ...
              tmp_ef->numeric_conditions_lh[tmp_ef->num_numeric_conditions]->lh_t = www->lh->lh_t;
	  ...
	  if ( ww->connective == COMP ) {
	    ...
	    tmp_ef->numeric_conditions_lh[0] = copy_Exp( ww->lh );
            ...
            tmp_ef->numeric_conditions_lh[0]->lh_t = ww->lh->lh_t;
	...
	if ( ww->connective == COMP ) {
	  ...
	  tmp_ef->numeric_conditions_lh[tmp_ef->num_numeric_conditions] = copy_Exp( ww->lh );
          ...
          tmp_ef->numeric_conditions_lh[tmp_ef->num_numeric_conditions]->lh_t =  ww->lh->lh_t;
    ...
    case COMP:
      ...
      tmp_ef->numeric_conditions_lh[0]->lh_t =  w->lh->lh_t;
