...
/*-----Scheduling-array-declaration----*/

OpConn *schedop_conn;
 OpConn *schedop_conn2;

 /* scheduling array index
   */


 int snum_schedop_conn;
...
/*    if (isTempTimeWindows==TRUE)
	break;
*/
#ifdef FASTER_HEURISTIC_IN_EXTERNAL
    if (gcmd_line.external && iterations++ > 30) 
	break;
#endif
  }
...
/*---scheduling-declaration-arrays-------*/

    schedop_conn = ( OpConn * ) calloc( gnum_op_conn *10, sizeof( OpConn ) );
    snum_schedop_conn=0;
    schedop_conn2 = ( OpConn * ) calloc( gnum_op_conn*10, sizeof( OpConn ) );

/*---End-of-scheduling-declaration-arrays--*/
...
/*----inverse--operators-array-for-scheduling--*/

   int g;
    for(g=0; g<snum_schedop_conn; g++) {

      schedop_conn2[g]= schedop_conn[snum_schedop_conn-g-1];
      }

/*--end-of-inverse-operators-array-for-scheduling--*/

/*----Sheduling-heuristic-calculation--*/
/* int g;
  printf("Node-expand-start               \n");
   for(g=0; g< snum_schedop_conn; g++) {

     printf("\n ");
      print_op_name( schedop_conn2[g].E[0])  ;
      }
      printf("\n                     ");*/


   if(snum_schedop_conn==0)
   {
       schedh=0;
   }
   if(snum_schedop_conn>0)
   {
   schedh=schedHeuristicPert(schedop_conn2);
   }
  

/*---End-of-sheduling-heuristic-calculation--*/

      if(graph_heuristic==TRUE)
	   return lh;
      if(sched_heuristic==TRUE)
          return schedh;
...

{
  ...
  for ( i = 0; i < gnum_flogic_goal; i++ ) {
    ft = gflogic_goal[i];
    if (gft_conn[ft].level < 0) continue;     
...

{
  ...
  int i, j, k, ft, min_p, min_e, ef=0, p;
  float val;
...

{
 ...
 /* load op in array schedop_conn*/

   schedop_conn[snum_schedop_conn++]= gop_conn[op];

  /* End of load op in array schedop_conn */
...
/*----scheduling-Heuristic--pert-function-*/


int schedHeuristicPert( OpConn *schedop_conn2){


int schedstart[snum_schedop_conn];

  int i=0;
  while ( i < snum_schedop_conn)
  {
    ...
    while ( j < i)
    {
	 ...
	 if (scheddepend(schedop_conn2[i],schedop_conn2[j])) {



	if (schedstart[j] + 1 > schedstart[i]) {

	  schedstart[i] = schedstart[j] + 1;
...
/*----End --of-scheduling-Heuristic--pert-function--*/



/*---scheduling-Heuristic--scheddepend-function--*/
...
/*-----------Numeric-dependence-test-----------*/
 ...
 for(i=0; i< gef_conn[p].num_f_PC; i++)
      {
          int x = gef_conn[p].f_PC_fl[i];

	  for(j=0;j< gef_conn[q].num_IN; j++)
	  {

              int y = gef_conn[q].IN_fl[j];
              if (x == y)
              {
		  return TRUE;
        ...
        for(j=0;j< gef_conn[q].num_AS; j++)
	  {

              int y = gef_conn[q].AS_fl[j];
              if (x == y)
              {
		  return TRUE;
     ...
     for(i=0; i< gef_conn[q].num_f_PC; i++)
         {
            int x = gef_conn[q].f_PC_fl[i];

	    for(j=0;j< gef_conn[p].num_IN; j++)
	     {

                int y = gef_conn[p].IN_fl[j];
                if (x == y)
                {
		    return TRUE;
           ...
           for(j=0;j< gef_conn[p].num_AS; j++)
	    {

                int y = gef_conn[p].AS_fl[j];
                if (x == y)
                {
		    return TRUE;
...
/*-End-of--scheduling-Heuristic--scheddepend-function--*/
