...
/*---scheduling-declaration-arrays-------*/
    ...
    schedop_conn = ( OpConn * ) calloc( gnum_op_conn, sizeof( OpConn ) );
    snum_schedop_conn=0;
    schedop_conn2 = ( OpConn * ) calloc( gnum_op_conn, sizeof( OpConn ) );
...
int schedstart[snum_schedop_conn];
  ...
  while ( i < snum_schedop_conn)
  {
    ...
    while ( j < i)
    {
	 ...
	 if (scheddepend(schedop_conn2[i].E[0],schedop_conn2[j].E[0])) {
...
Bool scheddepend(int p, int q){

    /*printf("Shahid: Using unsafe function scheddepend in relax.c \n");*/
    int i;
    int j;
