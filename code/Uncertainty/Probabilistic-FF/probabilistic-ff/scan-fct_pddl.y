...
%type <pPlNode> initial_literal_plus
%type <pPlNode> initial_literal
...
%type <pPlNode> atomic_formula_plus
...
initial_literal_plus  CLOSE_PAREN
{
...
PERCENT adl_goal_description  CLOSE_PAREN
{

  double rest;

  sscanf($4, "%lf", &(ggoal_probability));
  if ( ggoal_probability < 0 || ggoal_probability > 1 ) {
    printf("\n\nFCT parse: invalid goal probability %f\n\n", ggoal_probability);
    exit( 1 );
  }

  ggoal_percent = ( int ) (ggoal_probability * 100.0);
  rest = (ggoal_probability * 100.0) - ggoal_percent;
  if ( rest >= 0.5 ) {
    ggoal_percent++;
  }
...
/**********************************************************************/
initial_literal_plus:
initial_literal
{
  $$ = $1;
}
|
initial_literal initial_literal_plus
{
   $$ = $1;
   $$->next = $2;
}
...
/**********************************************************************/
initial_literal:
OPEN_PAREN  CPT_TOK atomic_formula_name PERCENT CLOSE_PAREN
{ 
  ...
  tmp = new_PlNode(CPT);
  tmp->atom = $3;
  sscanf($4, "%lf", &(tmp->cpt_p));
  if ( tmp->cpt_p <= 0 || tmp->cpt_p >= 1 ) {
    printf("\n\nFCT parse: invalid root node P %f\n\n", tmp->cpt_p);
    exit( 1 );
  }
/*   printf("\nparsed: %f", tmp->cpt_p); */
  tmp->sons = NULL;
...
|
OPEN_PAREN  CPT_TOK atomic_formula_name PERCENT OPEN_PAREN  AND_TOK  literal_name_plus CLOSE_PAREN CLOSE_PAREN
{ 
  /* unconditioned probability
   */
  PlNode *tmp;
...
|
OPEN_PAREN  OR_TOK  literal_name_plus  CLOSE_PAREN
{ 
  $$ = new_PlNode(OR);
  $$->sons = $3;
}
|
OPEN_PAREN  ONEOF_TOK  atomic_formula_plus  CLOSE_PAREN
{ 
  $$ = new_PlNode(ONEOF);
  $$->sons = $3;
}
|
OPEN_PAREN  MULTI_TOK  atomic_formula_plus  CLOSE_PAREN
{ 
  $$ = new_PlNode(MULTI);
  $$->sons = $3;
}
...
|
atomic_formula_name atomic_formula_plus
{
   $$  = new_PlNode(ATOM);
   $$->atom = $1;
   $$->next = $2;
