		...
		*wp = '\0';                                 // terminate string
		...
		if (strlen(word_buffer) != 0) {     // check if number is there
			...
			if (clause_vars.size() != 0 && 
			    (clause_vars.size() == clause_lits.size())) { //yeah, can add this clause
			    vector <int> temp;
...
//following lines will print out a solution if a solution exist
	...
	cout << "Memory out, unable to determine the satisfiablility of the instance";
	cout << endl;
	break;
    default:
...
int main(int argc, char ** argv)
{
	...
	cerr << "Copyright 2003-2004, Princeton Univ." << endl << endl;;
    ...
    cout << endl;
    cout << "Instance Cls: " << num_cls << "  MinCore Cls: " << num_cls - omit.size()
	 << " Diff: " << omit.size() << endl;
