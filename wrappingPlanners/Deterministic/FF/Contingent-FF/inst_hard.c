...
int_pointer lunknown_ini[MAX_PREDICATES];
...

{
   ...
   * initial, and unknown initial, state. for fast finding out 
   * if fact is in ini or not.
   */
  for ( i = 0; i < gnum_predicates; i++ ) {
    ...
    lunknown_ini[i] = ( int_pointer ) calloc( size, sizeof( int ) );
    for ( j = 0; j < size; j++ ) {
      lunknown_ini[i][j] = 0;
    }
    for ( j = 0; j < gnum_unknown_initial_predicate[i]; j++ ) {
      adr = instantiated_fact_adress( &gunknown_initial_predicate[i][j] );
      lunknown_ini[i][adr]++;
    }
  }
...

{
  ...
  case OR:
	...
	for ( i = 0; i < garity[w->fact->predicate]; i++ ) {
      ...
      if ( o->num_observe > 0 ) {
	tmp2->observe = ( Fact * ) calloc( o->num_observe, sizeof( Fact ) );
	tmp2->num_observe = o->num_observe;
	for ( i = 0; i < o->num_observe; i++ ) {
	  tmp2->observe[i].predicate = o->observe[i].predicate;
	  for ( j = 0; j < garity[tmp2->observe[i].predicate]; j++ ) {
	    tmp2->observe[i].args[j] = o->observe[i].args[j];
	    if ( tmp2->observe[i].args[j] < 0 &&
		 linst_table[DECODE_VAR( tmp2->observe[i].args[j] )] != -1 ) {
	      tmp2->observe[i].args[j] = linst_table[DECODE_VAR( tmp2->observe[i].args[j] )];
	    }
	  }
	}
      }
...

{
  ...
  for ( i = e; i; i = i->next ) {
    ...
    for ( l = i->effects; l; l = l->next ) {
      ...
      tt->is_nondeteff = l->is_nondeteff;
...

{
  ...
  if ( lunknown_ini[f->predicate][adr] > 0 ) {
    return TRUE;
  }
...

{
  ...
  e->adds_nondet = ( Bool * ) calloc( ma, sizeof( Bool ) );
  e->dels_nondet = ( Bool * ) calloc( md, sizeof( Bool ) );

  for ( l = ll; l; l = l->next ) {
    if ( l->negated ) {
      if ( l->is_nondeteff ) {
	e->dels_nondet[e->num_dels] = TRUE;
      } else {
	e->dels_nondet[e->num_dels] = FALSE;
      }
	...
	e->dels[e->num_dels].args[i] = ( l->fact.args[i] < 0 ) ?
      ...
      if ( l->is_nondeteff ) {
	e->adds_nondet[e->num_adds] = TRUE;
      } else {
	e->adds_nondet[e->num_adds] = FALSE;
      }
