...

{
  ...
  for ( o = gloaded_ops; o; o = o->next ) {
    ...
    if ( o->observe ) {
      tmp->observe = ( Fact * ) calloc( 1, sizeof( Fact ) );
      tmp->num_observe = 1;
      f = &(tmp->observe[0]);
      
      f->predicate = position_in_predicates_table( o->observe->atom->item );
      if ( f->predicate == -1 ) {
	...
	exit( 1 );
      ...
      m = 0;
      for ( t = o->observe->atom->next; t; t = t->next ) {
	if ( t->item[0] == '?' ) {
	  for ( i=tmp->num_vars-1; i>-1; i-- ) {
	  ...
	  if ( i == -1 ) {
	    printf("\nundeclared variable in literal. check input files\n\n");
	    exit( 1 );
	  }
	  ...
	  if ( (f->args[m] = position_in_constants_table( t->item )) == -1 ) {
	    printf("\nunknown constant in literal. check input files\n\n");
	    exit( 1 );
	  }
...

{
  ...
  for ( m = m->next; m; m = m->next ) {
    ...
    if ( m->connective == NOT ) {
      if ( m->is_nondeteff ) {
	tmp->effects->prev->is_nondeteff = TRUE;
      }
...

{
  ...
  /* we want these translated for h fn... at least we used to want this...
   */
  for ( i = 0; i < gnum_full_or_initial; i++ ) {
  ...
  for ( i = 0; i < gnum_operators; i++ ) {
    /* translate all observed predicates, for use in 0/1 h-fn
     */
    for ( j = 0; j < goperators[i]->num_observe; j++ ) {
      foo->fact = &(goperators[i]->observe[j]);
      while ( translate_one_negative_cond( foo, TRUE ) );
    }
...

{
  ...
  for ( i = 0; i < gnum_operators; i++ ) {
	...
	if ( goperators[i]->num_observe > 0 ) {
	  tmp_op->observe = ( Fact * ) calloc( goperators[i]->num_observe, sizeof( Fact ) );
	  tmp_op->num_observe = goperators[i]->num_observe;
	  for ( k = 0; k < goperators[i]->num_observe; k++ ) {
	    tmp_op->observe[k].predicate = goperators[i]->observe[k].predicate;
	    for ( j = 0; j < garity[tmp_op->observe[k].predicate]; j++ ) {
	      tmp_op->observe[k].args[j] = goperators[i]->observe[k].args[j];
	    }
	  }
	}
...

{
  ...
  for ( e = op->effects; e; e = e->next ) {
	...
	tmp_ef->adds_nondet = ( Bool * ) calloc( ma, sizeof( Bool ) );
	tmp_ef->dels_nondet = ( Bool * ) calloc( md, sizeof( Bool ) );
	for ( l = e->effects; l; l = l->next ) {
	  if ( l->negated ) {
	    if ( l->is_nondeteff ) {
	      tmp_ef->dels_nondet[tmp_ef->num_dels] = TRUE;
	    } else {
	      tmp_ef->dels_nondet[tmp_ef->num_dels] = FALSE;
	    }
	    tmp_ft = &(tmp_ef->dels[tmp_ef->num_dels++]);
	  } else {
	    if ( l->is_nondeteff ) {
	      tmp_ef->adds_nondet[tmp_ef->num_adds] = TRUE;
	    } else {
	      tmp_ef->adds_nondet[tmp_ef->num_adds] = FALSE;
	    }
