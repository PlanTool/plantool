...

{
   ...
   */
  for ( i = 0; i < gnum_operators; i++ ) {
    if ( !goperators[i]->axiom ) continue;
    gderived[goperators[i]->effects->effects->fact.predicate] = TRUE;
  }
  /* syntax check: no derived predicates in effects of normal ops!
   */ 
  for ( i = 0; i < gnum_operators; i++ ) {
    if ( goperators[i]->axiom ) continue;
    for ( ief = goperators[i]->effects; ief; ief = ief->next ) {
      for ( ilit = ief->effects; ilit; ilit = ilit->next ) {
	if ( gderived[ilit->fact.predicate] ) {
...

{
  ...
  for ( f = gpredicates_and_types; f; f = f->next ) {
    ...
    gderived[gnum_predicates++] = FALSE;
  }
...

{
  ...
  while ( introduce_derived_translate_op( ggoal ) );

  for ( i = 0; i < gnum_operators; i++ ) {
    while ( introduce_derived_translate_op( goperators[i]->preconds ) );
  ...
  if ( gcmd_line.display_info == 108 ) {
    ...
    for ( i = 0; i < gnum_operators; i++ ) {
      if ( goperators[i]->stratum != 0 ) continue;
      print_Operator( goperators[i] );
    }
    printf("\n\n");
    printf("\n\ndomain with new axiom ops is, (pre) stratum 1:");
    for ( i = 0; i < gnum_operators; i++ ) {
      if ( goperators[i]->stratum != 1 ) continue;
      print_Operator( goperators[i] );
    }
    printf("\n\n");
    fflush(stdout);
...

{
  ...
  gderived[gnum_predicates] = gderived[p];
  for ( j = 0; j < garity[p]; j++ ) {
...
Bool introduce_derived_translate_op( WffNode *w )

{

  static Bool fc = TRUE;
  static Bool had[MAX_PREDICATES];
   ...
   * (we keep the not-fts in ini in order to avoid
   ...
   */
  for ( j = 0; j < gnum_operators; j++ ) {
    if ( !goperators[j]->axiom ) continue;
    if ( goperators[j]->effects->effects->fact.predicate == p ) break;
  }
  if ( j == gnum_operators ) {
  ...
  tmpo = new_Operator( goperators[j]->name, goperators[j]->number_of_real_params );
  
  tmpo->axiom = TRUE;
  tmpo->stratum = 1;
  for ( k = 0; k < goperators[j]->num_vars; k++ ) {
    tmpo->var_names[k] = new_Token( strlen( goperators[j]->var_names[k] ) + 1 );
    strcpy( tmpo->var_names[k], goperators[j]->var_names[k] );
    tmpo->var_types[k] = goperators[j]->var_types[k];
  }
  tmpo->num_vars = goperators[j]->num_vars;
  
  tmpw = new_WffNode( NOT );
  tmpw->son = copy_Wff( goperators[j]->preconds );
  simplify_wff( &tmpw );
  ...
  tmpe = new_Effect();
  tmpe->conditions = copy_Wff( goperators[j]->effects->conditions );
  tmp = new_Literal();
  tmp->negated = TRUE;
  gis_deleted[p] = TRUE;
  tmp->fact.predicate = p;
  for ( k = 0; k < garity[p]; k++ ) {
    tmp->fact.args[k] = goperators[j]->effects->effects->fact.args[k];
  }
  tmpe->effects = tmp;
  tmpo->effects = tmpe;
  
  goperators[gnum_operators++] = tmpo;
...
void assign_axiom_strata( void )

{

  int i, j, k, level;
  int **R, *stratum, num_stratum;


  R = ( int ** ) calloc( gnum_operators, sizeof( int * ) );
  for ( i = 0; i < gnum_operators; i++ ) {
    R[i] = ( int * ) calloc( gnum_operators, sizeof( int ) );
    for ( j = 0; j < gnum_operators; j++ ) R[i][j] = 0;
  }
  stratum = ( int * ) calloc( gnum_operators, sizeof( int ) );
  ...
  for ( i = 0; i < gnum_operators; i++ ) {
    ...
    for ( j = 0; j < gnum_operators; j++ ) {
      if ( !goperators[j]->axiom ) continue;
      if ( 0 && goperators[i]->stratum == 0 &&
	   goperators[j]->stratum == 1 ) {
	R[i][j] = 2;
	continue;
      }
      if ( goperators[i]->stratum == 1 &&
	   goperators[j]->stratum == 0 ) {
	continue;
      }
      if ( goperators[i]->stratum == 1 &&
	   goperators[j]->stratum == 1 ) {
  ...
  for ( j = 0; j < gnum_operators; j++ ) {
	...
	if ( !goperators[k]->axiom ) continue;
	if ( R[i][j] > 0 && R[j][k] > 0 ) { 
  ...
  for ( i = 0; i < gnum_operators; i++ ) {
    if ( !goperators[i]->axiom ) {
      goperators[i]->stratum = -1;
      continue;
    }
    goperators[i]->stratum = 0;
  }
  ...
  while ( TRUE ) {
     ...
     */
    for ( j = 0; j < gnum_operators; j++ ) {
      if ( goperators[j]->stratum == 0 ) break;
    }
    if ( j == gnum_operators ) break;/* no such j: stop */

    /* build new stratum
     */
    num_stratum = 0;
    for ( j = 0; j < gnum_operators; j++ ) {
      if ( goperators[j]->stratum != 0 ) continue;
      for ( i = 0; i < gnum_operators; i++ ) {
	if ( goperators[i]->stratum != 0 ) continue;
	if ( R[i][j] == 2 ) break;
      }
      if ( i < gnum_operators ) continue;
      stratum[num_stratum++] = j;
    }
    for ( i = 0; i < num_stratum; i++ ) {
      goperators[stratum[i]]->stratum = level;
    }
    level++;
  }
  gnum_strata = level;
...
Bool i_effect_occurs_negatively_in_jpre( int i, int j )

{
  ...
  p = goperators[i]->effects->effects->fact.predicate;

  w = goperators[j]->preconds;

  result = p_occurs_negatively_in_w( p, w );

  if ( gcmd_line.debug >= 3 ) {
    if ( result || gcmd_line.debug >= 4 ) printf("\n\nderived predicate of OP %d, %s, ", i, gpredicates[p]);
    if ( result ) {
      printf("occurs negatively in prec OP %d", j);
    } else {
      if ( gcmd_line.debug >= 4 ) printf("does not occur negatively in prec OP %d", j);
    }
  }
...
Bool p_occurs_negatively_in_w( int p, WffNode *w )
...
Bool i_effect_occurs_positively_in_jpre( int i, int j )

{
  ...
  if ( gcmd_line.debug >= 3 ) {
    if ( result || gcmd_line.debug >= 4 ) printf("\n\nderived predicate of OP %d, %s, ", i, gpredicates[p]);
    if ( result ) {
      printf("occurs positively in prec OP %d", j);
    } else {
      if ( gcmd_line.debug >= 4 ) printf("does not occur positively in prec OP %d", j);
    }
  }
...
Bool p_occurs_positively_in_w( int p, WffNode *w )
