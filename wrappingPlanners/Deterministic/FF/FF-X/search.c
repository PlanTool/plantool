...

{
  ...
  if ( !ltakeA ) {
    ...
    get_naxA( &(lehc_current_start->S) );
    for ( i = 0; i < gnum_naxA; i++ ) {
      g = result_to_dest( &S_, &(lehc_current_start->S), gnaxA[i] );
      add_to_ehc_space( &S_, gnaxA[i], lehc_current_start, g );
    }
  }
...

{
  ...
  while ( TRUE ) {
    ...
    for ( i = 0; i < gnum_naxA; i++ ) {
      /* DEBUG
       */
      if ( gop_conn[gnaxA[i]].axiom ) {
	  printf("\nreal A axiom??\n\n");
	  print_op_name( gnaxA[i] );
	  exit( 1 );
      }
      result_to_dest( &S, &(first->S), gnaxA[i] );
      add_to_bfs_space( &S, gnaxA[i], first );
    }
...

{
  ...
  for ( i = 0; i < source->num_F; i++ ) {
     ...
     */
    if ( gft_conn[source->F[i]].axiom_add ||
	 gft_conn[source->F[i]].axiom_del ) {
	continue;
    }
...

{
	...
	for ( i = 0; i < gnum_axA; i++ ) {
	  /* DEBUG
	   */
	  if ( !gop_conn[gaxA[i]].axiom ) {
	    printf("\naxiom A no axiom??\n\n");
	    exit( 1 );
	  }
	  if ( gop_conn[gaxA[i]].stratum != n ) {
	    continue;
	  }
	  if ( gcmd_line.debug >= 3 ) {
	    printf("\n----------------------------------apply axiom ");
	  ...
	  ft = gef_conn[gop_conn[gaxA[i]].E[0]].A[0];
	  if ( in_dest[ft] ) {
	    continue;
	  }
	  changes = TRUE;
	  ...
	  if ( gef_conn[gop_conn[gaxA[i]].E[0]].num_D > 0 ) {
	    ft = gef_conn[gop_conn[gaxA[i]].E[0]].D[0];
	    if ( !in_dest[ft] ) continue;
	    changes = TRUE;
	    if ( gcmd_line.debug >= 3 ) {
	      printf(" DEL! ");
	      print_ft_name( ft );
