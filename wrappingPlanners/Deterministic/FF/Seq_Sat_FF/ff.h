...
#define ARTIFICIAL_GTT_VALUE 0.01
#define EPSILON ( ARTIFICIAL_GTT_VALUE / 100 )
...
typedef struct _OpConn {
  ...
  /* ugly hack - emil 
   * assumes that numeric effects that appear in the metric 
   * do not appear in WHEN clauses of actions 
   * in other words, no state dependent costs for actions. */
  float cost;
...
#define EHC_HELPFUL_ACTION_STORAGE_CAPACITY 8192
...
extern Bool gcompetition_output;
extern char gcompetition_output_file[];
