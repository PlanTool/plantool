...

{
  ...
  if ( ggoal->connective == TRU ) {
    printf("\nff: inst_final.c: goal can be simplified to TRUE. The empty plan solves it\n\n");
    gnum_plan_ops = 0;
    exit( 1 );
  }
  if ( ggoal->connective == FAL ) {
    printf("\nff: inst_final.c: goal can be simplified to FALSE. No plan will solve it\n\n");
    exit( 1 );
  }
...

{
  ...
  while ( a ) {
    if ( a->norm_operator ) {
      ...
      /* no is the operator, a the instantiated act */
      no = a->norm_operator;
	...
	aa->nonconditional = ne->nonconditional;
...

{
  ...
  /* temp for # of unconditional effects for each action */
  int num_uc, num_uc_adds, num_uc_dels;
  /* the unconditional effects */
  int *uc_efs = calloc(lnum_effects, sizeof(int));
  /* the unconditional adds, dels */
  int *uc_adds = calloc(gnum_relevant_facts, sizeof(int));
  int *uc_dels = calloc(gnum_relevant_facts, sizeof(int));
  ...
  for ( a = gactions; a; a = a->next ) {
    ...
    /* here we find the nonconditional effects */
    /* also set cost to be nonconditional cost */
    num_uc = num_uc_adds = num_uc_dels = 0;
    gop_conn[n_op].cost = 0;
    
    for ( i = 0; i < a->num_effects; i++ ) {
      e = &(a->effects[i]);
      if (e->nonconditional && !(e->removed)) {
	uc_efs[num_uc++] = i;
	gop_conn[n_op].cost += e->cost;

	for(j = 0; j < e->num_adds; j++) {
	  if( ! array_contains_int(uc_adds, num_uc_adds, e->adds[j])) {
	    uc_adds[num_uc_adds++] = e->adds[j];
	  }
	}
	for(j = 0; j < e->num_dels; j++) {
	  if( ! array_contains_int(uc_dels, num_uc_dels, e->dels[j])) {
	    uc_dels[num_uc_dels++] = e->dels[j];
	  }
	}
      }
    }
    ...
    if(num_uc > 1) {
      printf("WARNING: > 1 nonconditional effects for action %s\n", gop_conn[n_op].action->name );
      for ( i = 0; i < a->num_effects; i++ ) {
	e = &(a->effects[i]);
	printf("effect %d (%s):\n", i, e->removed ? "REMOVED" : "VALID");
	for(j = 0; j < e->num_conditions; j++) {
	  printf("\tcondition "); print_ft_name(e->conditions[j]); printf("\n");
	}
	for(j = 0; j < e->num_adds; j++) {
	  printf("\tadd "); print_ft_name(e->adds[j]); printf("\n");
	}
	for(j = 0; j < e->num_dels; j++) {
	  printf("\tdel "); print_ft_name(e->dels[j]); printf("\n");
	}
      }
      printf("\n\n");
    }
    ...
    for ( i = 0; i < a->num_effects; i++ ) {
      e = &(a->effects[i]);
      gef_conn[n_ef].cost = gop_conn[n_op].cost;
      if(!e->nonconditional) {
	...
	continue;
      ...
      /* new line to store effect index in op */
      gef_conn[n_ef].ef_index = gop_conn[n_op].num_E++;
	...
	continue;
      ...
      /*****************************EFFECTS********************************/
      gef_conn[n_ef].A = ( int * ) calloc( e->num_adds + num_uc_adds, sizeof( int ) );
      gef_conn[n_ef].D = ( int * ) calloc( e->num_dels + num_uc_dels, sizeof( int ) );
	...
	if ( k < gef_conn[n_ef].num_PC ) continue;
	/* don't add it if it's an unconditional effect. */
	if(array_contains_int(uc_adds, num_uc_adds, e->adds[j])) continue;
	gef_conn[n_ef].A[gef_conn[n_ef].num_A++] = e->adds[j];
	/*
	printf("in first loop effect number %d, added add", n_ef);
	print_ft_name(e->adds[j]);
	printf("\n");*/
      }

      /* now add unconditional adds */
      for( j = 0; j < num_uc_adds; j++) {
	gef_conn[n_ef].A[gef_conn[n_ef].num_A++] = uc_adds[j];
	/*
	printf("in second loop effect number %d, added add", n_ef);
	print_ft_name(uc_adds[j]);
	printf("\n");*/
	...
	/* don't add it if it's an unconditional delete. */
	if(array_contains_int(uc_dels, num_uc_dels, e->dels[j])) continue;
	gef_conn[n_ef].D[gef_conn[n_ef].num_D++] = e->dels[j];
      }

      /* now add unconditional dels */
      for( j = 0; j < num_uc_dels; j++) {
	gef_conn[n_ef].D[gef_conn[n_ef].num_D++] = uc_dels[j];
      }
