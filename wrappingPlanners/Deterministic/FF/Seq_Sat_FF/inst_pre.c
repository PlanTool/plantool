...

{
  ...
  if ( ggoal->connective == TRU ) {
    printf("\nff: inst_pre.c: goal can be simplified to TRUE. The empty plan solves it\n\n");
    gnum_plan_ops = 0;
    exit( 1 );
  }
  if ( ggoal->connective == FAL ) {
    
    printf("\nff: inst_pre.c: goal can be simplified to FALSE. No plan will solve it\n\n");
    exit( 1 );
  }
...

{
  ...
  for ( i = 0; i < gnum_operators; i++ ) {
    if ( goperators[i]->hard ) {
      ...
      printf("inst_pre.c: split_domain: hard operator, find out when this happens\n");
      exit(-1);
...

{
  ...
  for ( e = op->effects; e; e = e->next ) {
	...
	(*nop)->effects->prev = tmp_ef;
      ...
      /* here we note whether the effect is not conditional 
       * (since the connective is TRU */
      
      tmp_ef->nonconditional = TRUE;
