...
int gnum_tsp_vars;
TSP_Var_pointer gtsp_vars[MAX_TSP_VARS];
...
/* dirtier: true if we set total-time to true to force 
...
/* do output for competition */

Bool gcompetition_output = FALSE;
...
char gcompetition_output_file[MAX_LENGTH];
...
int run( int argc, char * argv[] )
{
  ...
  sprintf(gcompetition_output_file, "%s.solution", gcmd_line.fct_file_name);
   ...
   */
  check_assigncycles();
  /* set the relevance info (in expressions.c)
   */
  ...
  /*
  printf("\nabout to call compute_a_distances\n");
  compute_a_distances();*/

  /*  compute_landmarks(&ginitial_state);*/
/*   printf("\n"); */
/*   print_all_landmarks(); */
/*   printf("\n\n"); */
  ...
  /*  for(i = 0; i < gnum_ef_conn; i++) {
    printf("action: "); print_op_name(gef_conn[i].op);
    printf("\tpre: "); 
  ...
  printf("display info is %d", gcmd_line.display_info);
  ...
  if ( gcmd_line.ehc ) {
    ...
    initial_cost = eval(&ginitial_state, &i, FALSE);

    if ( gcmd_line.display_info ) {
      printf("Real initial cost %f, with %d actions\n", initial_cost, i);
    }


    if(initial_cost == INFINITY) {
      printf("Problem unsolvable, exiting.\n");
      exit(0);
    }
...

{
  ...
  printf("-S          Use set-additive heuristic rather than duplicate-eliminated additive\n");
  printf("-T          Use TSP heuristic\n");
  printf("-H          Use standard additive heuristic with wA*\n\n");

  printf("-C          Competition output\n\n");
...

{
  ...
  memset(gcompetition_output_file, 0, MAX_LENGTH);
  ...
  while ( --argc && ++argv ) {
    ...
    case 'M':
      ...
      break;
    case 'R':
      gcmd_line.ratio = TRUE;
      break;
    case 'L':
      gcmd_line.logic = TRUE;
      break;
    case 'T':
      gcmd_line.tsp = TRUE;
      gcmd_line.hadd_de = FALSE;
      break;
    case 'H':
      gcmd_line.hadd = TRUE;
      /* ehc is off with hadd */
      gcmd_line.ehc = FALSE;
      gcmd_line.hadd_de = FALSE;
      break;
    case 'S':
      ...
      break;
    case 'X':
      gcmd_line.mutexes = TRUE;
      break;
    case 'C':
      gcompetition_output = TRUE;
      break;
    default:
      if ( --argc && ++argv ) {
	switch ( option ) {
  ...
  /* default setting is gain */
  if( !gcmd_line.logic && !gcmd_line.min && !gcmd_line.ratio) {
    gcmd_line.gain = TRUE;
  }
