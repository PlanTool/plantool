...

{
  ...
  printf("\n\nEffects:");
  for ( e = o->effects; e; e = e->next ) {
    printf("\n\neffect %d, parameters %d (%s)", m++, e->num_vars, 
	   e->nonconditional ? "normal" : "conditional");
...
void print_op_name( int index ) {
  ...
  fprint_op_name(stdout, index );

}

void fprint_op_name(  FILE *file, int index )

{
  ...
  if ( !a->norm_operator &&
       !a->pseudo_action ) {
    fprintf(file, "REACH-GOAL");
  } else {
    fprintf(file, "%s", a->name ); 
    for ( i = 0; i < a->num_name_vars; i++ ) {
      fprintf(file, " %s", gconstants[a->name_inst_table[i]]);
    }
  }
...
{  
  ...
  FILE *comp_output_ = NULL;

  if ( gcmd_line.display_info ) {
    printf("\n\nff: found legal plan (steps: %d) as follows\n", gnum_plan_ops);
  }

  if(gcompetition_output) {
    comp_output_ = fopen(gcompetition_output_file, "w");
  }
  ...
  for ( i = 0; i < gnum_plan_ops; i++ ) {
    ...
    if(gcompetition_output) {
      fprintf(comp_output_,"%4d: ", i);
      fprintf(comp_output_,"(");
      fprint_op_name(comp_output_, gplan_ops[i] );
      fprintf(comp_output_,")");
    }
    ...
    /*    if(gop_conn[gplan_ops[i]].num_E != 1) {
      ...
      /* we are actually getting cost of operator
       * and not effect, handled in utility.c.
       * this assumes uniform costs for all effects 
       * of an operator. */

      cost += get_cost(gop_conn[gplan_ops[i]].E[0]);
      if(artificial_gtt && gcmd_line.display_info) {
	printf(" (%f) ", get_cost(gop_conn[gplan_ops[i]].E[0]) );
	printf("(%f artificial)", ARTIFICIAL_GTT_VALUE);
      }
      /*    }*/
      printf("\n");
      if(gcompetition_output) {
