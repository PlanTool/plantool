...
Bool search_for_better_state( State *S, float h, int num_actions, State *S_, float *h_, int *num_actions_ );

void add_to_ehc_space( State *S, int op, EhcNode *father );
/* int expand_first_node( int h ); */
/*float expand_first_node( float h );*/
float expand_first_node( float h, int *n_acts, int *op, EhcNode **expanded);



void hash_ehc_node( EhcNode *n );
Bool ehc_state_hashed( State *S );
Bool superior_ehc_state_hashed( State *S, float g );
