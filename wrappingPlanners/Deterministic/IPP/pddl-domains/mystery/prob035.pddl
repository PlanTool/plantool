(DEFINE (PROBLEM MYSTY-35)
   (:DOMAIN MYSTERY-TYPED)
   (:OBJECTS PEAR POPOVER GRAPEFRUIT PORK ENDIVE ORANGE POTATO COD
             YOGURT MUTTON TURKEY HAROSET KALE CUCUMBER PAPAYA APPLE
             MARZIPAN SNICKERS GUAVA WONDERBREAD ONION SCALLOP BROCCOLI TUNA
             WURST SCALLION MUFFIN MELON HAMBURGER PEPPER SHRIMP RICE
             TOMATO CHOCOLATE CHICKEN PEA FLOUNDER LOBSTER HAM - FOOD
             LOVE INTOXICATION EXCITEMENT REST CURIOSITY SATISFACTION
             EXPECTATION STIMULATION ACHIEVEMENT LEARNING UNDERSTANDING
             - PLEASURE
             HANGOVER ANXIETY BOILS ANGINA JEALOUSY LACERATION ANGER
             ABRASION GRIEF DEPRESSION LONELINESS DREAD - PAIN
             GUANABARA PENNSYLVANIA BAVARIA - PROVINCE
             JUPITER VULCAN EARTH URANUS - PLANET)
   (:INIT (CRAVES JEALOUSY TUNA)
          (EATS ONION SHRIMP)
          (CRAVES LACERATION TUNA)
          (EATS TOMATO WONDERBREAD)
          (EATS CHICKEN HAM)
          (LOCALE APPLE PENNSYLVANIA)
          (LOCALE RICE PENNSYLVANIA)
          (HARMONY SATISFACTION VULCAN)
          (EATS HAROSET MUTTON)
          (LOCALE FLOUNDER BAVARIA)
          (EATS SCALLION SNICKERS)
          (EATS PEPPER BROCCOLI)
          (HARMONY UNDERSTANDING URANUS)
          (EATS TUNA HAMBURGER)
          (LOCALE KALE PENNSYLVANIA)
          (LOCALE SNICKERS GUANABARA)
          (EATS CUCUMBER SCALLOP)
          (CRAVES HANGOVER POTATO)
          (EATS CUCUMBER YOGURT)
          (EATS ORANGE PEAR)
          (HARMONY EXPECTATION EARTH)
          (LOCALE PEPPER BAVARIA)
          (LOCALE SCALLOP GUANABARA)
          (LOCALE WURST PENNSYLVANIA)
          (EATS MUTTON APPLE)
          (LOCALE PEAR PENNSYLVANIA)
          (CRAVES EXCITEMENT POTATO)
          (CRAVES LEARNING MELON)
          (EATS PEPPER WURST)
          (CRAVES CURIOSITY APPLE)
          (EATS POTATO GRAPEFRUIT)
          (CRAVES STIMULATION BROCCOLI)
          (EATS APPLE POPOVER)
          (EATS TURKEY APPLE)
          (EATS SHRIMP FLOUNDER)
          (EATS WURST CHOCOLATE)
          (EATS SNICKERS SCALLION)
          (EATS RICE MELON)
          (LOCALE SHRIMP GUANABARA)
          (EATS MARZIPAN TURKEY)
          (EATS HAM CHICKEN)
          (EATS ONION LOBSTER)
          (HARMONY REST EARTH)
          (LOCALE CHOCOLATE PENNSYLVANIA)
          (EATS TOMATO SCALLION)
          (EATS HAM MUFFIN)
          (EATS POPOVER PORK)
          (EATS HAROSET PAPAYA)
          (LOCALE SCALLION GUANABARA)
          (LOCALE MARZIPAN PENNSYLVANIA)
          (LOCALE PEA GUANABARA)
          (EATS WURST PEPPER)
          (EATS BROCCOLI HAM)
          (ORBITS VULCAN EARTH)
          (LOCALE MUTTON BAVARIA)
          (HARMONY ACHIEVEMENT URANUS)
          (EATS LOBSTER ONION)
          (HARMONY CURIOSITY VULCAN)
          (EATS MELON CHOCOLATE)
          (EATS BROCCOLI PEPPER)
          (CRAVES EXPECTATION SCALLOP)
          (EATS APPLE GRAPEFRUIT)
          (EATS GRAPEFRUIT POTATO)
          (EATS GUAVA RICE)
          (EATS PEA PEPPER)
          (EATS YOGURT TURKEY)
          (EATS PORK CUCUMBER)
          (EATS ORANGE POPOVER)
          (EATS WONDERBREAD CHICKEN)
          (CRAVES UNDERSTANDING CHICKEN)
          (EATS CHOCOLATE SCALLION)
          (EATS SCALLION TOMATO)
          (EATS COD CUCUMBER)
          (EATS POPOVER APPLE)
          (EATS FLOUNDER SHRIMP)
          (LOCALE TOMATO BAVARIA)
          (EATS KALE TURKEY)
          (EATS ENDIVE POTATO)
          (LOCALE TUNA GUANABARA)
          (EATS FLOUNDER LOBSTER)
          (EATS SHRIMP ONION)
          (EATS APPLE MUTTON)
          (EATS SCALLOP CHOCOLATE)
          (EATS TURKEY YOGURT)
          (CRAVES DREAD PEA)
          (EATS MARZIPAN KALE)
          (EATS CHOCOLATE WURST)
          (HARMONY INTOXICATION URANUS)
          (CRAVES REST KALE)
          (EATS SCALLOP BROCCOLI)
          (EATS HAMBURGER SNICKERS)
          (EATS BROCCOLI MUFFIN)
          (EATS COD PAPAYA)
          (EATS CHOCOLATE SCALLOP)
          (EATS HAMBURGER TOMATO)
          (LOCALE CHICKEN BAVARIA)
          (EATS CHOCOLATE HAMBURGER)
          (EATS PAPAYA HAROSET)
          (LOCALE PAPAYA GUANABARA)
          (EATS SNICKERS HAMBURGER)
          (EATS RICE CHICKEN)
          (EATS TOMATO SHRIMP)
          (EATS MUTTON PEAR)
          (CRAVES ABRASION HAMBURGER)
          (ORBITS JUPITER VULCAN)
          (EATS HAROSET CUCUMBER)
          (LOCALE HAROSET GUANABARA)
          (CRAVES GRIEF HAMBURGER)
          (EATS POTATO ENDIVE)
          (EATS PORK POPOVER)
          (EATS CHOCOLATE MELON)
          (LOCALE GUAVA GUANABARA)
          (EATS GUAVA SCALLION)
          (EATS MUFFIN PEPPER)
          (ORBITS EARTH URANUS)
          (LOCALE ENDIVE PENNSYLVANIA)
          (LOCALE BROCCOLI PENNSYLVANIA)
          (EATS TOMATO HAMBURGER)
          (EATS TUNA FLOUNDER)
          (HARMONY EXCITEMENT VULCAN)
          (CRAVES LOVE POPOVER)
          (EATS HAMBURGER CHOCOLATE)
          (LOCALE HAM GUANABARA)
          (EATS TOMATO LOBSTER)
          (CRAVES ANGER TUNA)
          (LOCALE POTATO GUANABARA)
          (EATS YOGURT CUCUMBER)
          (LOCALE POPOVER GUANABARA)
          (EATS MUTTON HAROSET)
          (LOCALE CUCUMBER PENNSYLVANIA)
          (EATS MUFFIN HAM)
          (LOCALE GRAPEFRUIT PENNSYLVANIA)
          (CRAVES SATISFACTION GUAVA)
          (EATS YOGURT ENDIVE)
          (HARMONY STIMULATION EARTH)
          (CRAVES ACHIEVEMENT WURST)
          (EATS CHICKEN WONDERBREAD)
          (CRAVES INTOXICATION ORANGE)
          (LOCALE PORK GUANABARA)
          (LOCALE YOGURT GUANABARA)
          (EATS PEPPER PEA)
          (EATS GRAPEFRUIT APPLE)
          (EATS SCALLION CHOCOLATE)
          (EATS TURKEY MARZIPAN)
          (CRAVES LONELINESS CHICKEN)
          (LOCALE TURKEY PENNSYLVANIA)
          (EATS YOGURT KALE)
          (EATS MELON RICE)
          (EATS WONDERBREAD TOMATO)
          (EATS SHRIMP TOMATO)
          (LOCALE HAMBURGER PENNSYLVANIA)
          (LOCALE ORANGE GUANABARA)
          (EATS SCALLOP CUCUMBER)
          (EATS PAPAYA COD)
          (EATS CUCUMBER KALE)
          (EATS HAMBURGER TUNA)
          (EATS BROCCOLI SCALLOP)
          (EATS MUFFIN BROCCOLI)
          (HARMONY LEARNING URANUS)
          (EATS PEAR ORANGE)
          (EATS POPOVER ORANGE)
          (LOCALE WONDERBREAD GUANABARA)
          (LOCALE LOBSTER BAVARIA)
          (EATS CUCUMBER PORK)
          (EATS SCALLION GUAVA)
          (EATS MELON HAM)
          (CRAVES ANXIETY COD)
          (EATS PEAR MUTTON)
          (EATS KALE YOGURT)
          (EATS KALE MARZIPAN)
          (EATS PEA MUFFIN)
          (LOCALE MELON GUANABARA)
          (LOCALE ONION BAVARIA)
          (HARMONY LOVE URANUS)
          (LOCALE COD GUANABARA)
          (EATS HAM MELON)
          (EATS ORANGE HAROSET)
          (EATS APPLE TURKEY)
          (EATS PEPPER MUFFIN)
          (EATS TURKEY KALE)
          (EATS LOBSTER FLOUNDER)
          (CRAVES DEPRESSION PEPPER)
          (EATS ENDIVE YOGURT)
          (CRAVES BOILS COD)
          (CRAVES ANGINA SCALLOP)
          (EATS FLOUNDER TUNA)
          (LOCALE MUFFIN GUANABARA)
          (EATS HAROSET ORANGE)
          (EATS CHICKEN RICE)
          (EATS CUCUMBER HAROSET)
          (ATTACKS GUANABARA PENNSYLVANIA)
          (EATS RICE GUAVA)
          (EATS KALE CUCUMBER)
          (EATS CUCUMBER COD)
          (EATS LOBSTER TOMATO)
          (EATS HAM BROCCOLI)
          (ATTACKS PENNSYLVANIA BAVARIA)
          (EATS MUFFIN PEA))
   (:GOAL (AND (CRAVES LONELINESS HAMBURGER)
               (CRAVES GRIEF HAMBURGER)
               (CRAVES HANGOVER ONION)
               (CRAVES ANGINA MELON))))