...
action_def_body
|
OBSERVE_TOK  OPEN_PAREN  predicate  term_star  CLOSE_PAREN
{ 
  scur_op->observe = new_PlNode(ATOM);
  scur_op->observe->atom = new_TokenList();
  scur_op->observe->atom->item = $3;
  scur_op->observe->atom->next = $4;
}
action_def_body
|
...
|
OPEN_PAREN NONDET_TOK  literal_term CLOSE_PAREN
{ 
  if ( sis_negated ) {
    $$ = new_PlNode(NOT);
    $$->sons = new_PlNode(ATOM);
