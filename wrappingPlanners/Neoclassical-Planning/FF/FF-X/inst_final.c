...

{
  ...
  if ( ggoal->connective == TRU ) {
    printf("\nff: final: ggoal can be simplified to TRUE. The empty plan solves it\n\n");
    exit( 1 );
  }
...

{
  ...
  for ( i = 0; i < gnum_ft_conn; i++ ) {
    ...
    gft_conn[i].num_axPC = 0;
    gft_conn[i].num_naxPC = 0;

    gft_conn[i].axiom_add = FALSE;
    gft_conn[i].axiom_del = FALSE;

    gft_conn[i].rand = random() % BIG_INT;
  }
  gaxdels = ( int * ) calloc( gnum_ft_conn, sizeof( int ) );
  gnum_axdels = 0;
  ...
  for ( a = gactions; a; a = a->next ) {
    ...
    gop_conn[n_op].stratum = a->stratum;
    ...
    if ( gop_conn[n_op].stratum == 0 ) {
      gef_conn[gop_conn[n_op].E[0]].num_D = 0;
    }
	...
	for ( j = 0; j < gef_conn[ef].num_PC; j++ ) {
	  ...
	  if ( gop_conn[n_op].axiom ) {
	      gft_conn[gef_conn[ef].PC[j]].num_axPC++;
	  } else {
	      gft_conn[gef_conn[ef].PC[j]].num_naxPC++;
	  }
	}
 	for ( j = 0; j < gef_conn[ef].num_A; j++ ) {
	  gft_conn[gef_conn[ef].A[j]].num_A++;
	  if ( gop_conn[n_op].axiom ) {
	      gft_conn[gef_conn[ef].A[j]].axiom_add = TRUE;
	  }
	}
	for ( j = 0; j < gef_conn[ef].num_D; j++ ) {
	  gft_conn[gef_conn[ef].D[j]].num_D++;
	  if ( gop_conn[n_op].axiom ) {
	      gft_conn[gef_conn[ef].D[j]].axiom_del = TRUE;
	  }
	}
      }
    }
  ...
  for ( i = 0; i < gnum_ft_conn; i++ ) {
    ...
    gft_conn[i].num_PC = 0;
    if ( gft_conn[i].num_axPC > 0 ) {
      gft_conn[i].axPC = ( int * ) calloc( gft_conn[i].num_axPC, sizeof( int ) );
    }
    gft_conn[i].num_axPC = 0;
    if ( gft_conn[i].num_naxPC > 0 ) {
      gft_conn[i].naxPC = ( int * ) calloc( gft_conn[i].num_naxPC, sizeof( int ) );
    }
    gft_conn[i].num_naxPC = 0;
    ...
    if ( gft_conn[i].axiom_del ) {
	gaxdels[gnum_axdels++] = i;
    }
  }
  for ( i = 0; i < ggoal_state.num_F; i++ ) {
  ...
  for ( i = 0; i < gnum_ef_conn; i++ ) {
	...
	for ( j = 0; j < gef_conn[i].num_PC; j++ ) {
	    gft_conn[gef_conn[i].PC[j]].axPC[gft_conn[gef_conn[i].PC[j]].num_axPC++] = i;
	}
    } else {
	for ( j = 0; j < gef_conn[i].num_PC; j++ ) {
	    gft_conn[gef_conn[i].PC[j]].naxPC[gft_conn[gef_conn[i].PC[j]].num_naxPC++] = i;
	}
    }
  ...
  if ( gcmd_line.display_info == 121 ) {
    ...
    printf("\n\n------------------OP ARRAY %d:-----------------------",
	   gnum_op_conn);
    for ( i = 0; i < gnum_op_conn; i++ ) {
      printf("\n\nOP %d, stratum %d: ", i, gop_conn[i].stratum);
      if ( gop_conn[i].axiom ) printf("(axiom) ");
      print_op_name( i );
      printf("\n----------EFFS %d:", gop_conn[i].num_E);fflush(stdout);
      for ( j = 0; j < gop_conn[i].num_E; j++ ) {
	printf("\neffect %d", gop_conn[i].E[j]);fflush(stdout);
      }
    }
    ...
    for ( i = 0; i < gnum_ft_conn; i++ ) {
      ...
      printf(" --------- AXIOM ADD %d, AXIOM DEL %d", 
	     gft_conn[i].axiom_add,
	     gft_conn[i].axiom_del);
      printf("\n----------PRE COND OF:");
      for ( j = 0; j < gft_conn[i].num_PC; j++ ) {
	printf("\neffect %d", gft_conn[i].PC[j]);
      }
      printf("\n----------AXIOM PRE COND OF:");
      for ( j = 0; j < gft_conn[i].num_axPC; j++ ) {
	printf("\neffect %d", gft_conn[i].axPC[j]);
      }
      printf("\n----------NON-AXIOM PRE COND OF:");
      for ( j = 0; j < gft_conn[i].num_naxPC; j++ ) {
	printf("\neffect %d", gft_conn[i].naxPC[j]);
      }
      printf("\n----------ADD BY:");
