 ...
 * Description: modified from inst_pre.c in LPG
...
void transfer_PlDP_PlOperator()
{
  ...
  for ( prev_o = gloaded_ops; prev_o->next; prev_o = prev_o->next );
  
  for (dp=gloaded_dps;dp;dp=dp->next,i++) 
  {
        // name of DP-operator
        strcpy(DPname,"deripred-");
        if (dp->preconds->connective == AND) {
            for (type_num=0,n=dp->preconds->sons;n;n=n->next,type_num++)
            {
                    strcat(DPname,n->atom->item);
                     strcat(DPname,"=");
            }   
        }  else {
            strcat(DPname, dp->preconds->atom->item); 
                     strcat(DPname,"=");
        }          
         strcat(DPname,">"); 
        strcat(DPname, dp->atom->item);
        
        // create 
        o = new_PlOperator(DPname);
        o->parse_params = NULL;
        o->number_of_real_params = 0;
        o->params = NULL;

        // precond
        o->preconds = dp->preconds;        
        
        // effect
        o->effects = new_PlNode(AND);   
        o->effects->parse_vars = NULL;
        o->effects->next = NULL;    
        o->effects->value = 0.0;
        o->effects->atom = NULL;
        
        o->effects->sons = new_PlNode(ATOM);
        o->effects->sons->parse_vars = NULL;
        o->effects->sons->next = NULL;
        o->effects->sons->value = 0.0;
        o->effects->sons->atom = dp->atom;
        o->effects->sons->sons = NULL;
        ...
        o->effects->sons = new_PlNode(WHEN);
        o->effects->sons->parse_vars = NULL;
        o->effects->sons->next = NULL;
        o->effects->sons->value = 0.0;
        o->effects->sons->atom = NULL;
        
        o->effects->sons->sons = new_PlNode(TRU);
        o->effects->sons->sons->parse_vars = NULL;
        o->effects->sons->sons->value = 0.0;
        o->effects->sons->sons->atom = NULL;
        o->effects->sons->sons->sons = NULL;
        
        o->effects->sons->sons->next = new_PlNode(AND);
        o->effects->sons->sons->next->parse_vars = NULL;
        o->effects->sons->sons->next->value = 0.0;
        o->effects->sons->sons->next->atom = NULL;
        o->effects->sons->sons->next->next = NULL;
        
        o->effects->sons->sons->next->sons = new_PlNode(ATOM);
        o->effects->sons->sons->next->sons->parse_vars = NULL;
        o->effects->sons->sons->next->sons->value = 0.0;
        o->effects->sons->sons->next->sons->atom = dp->atom;
        o->effects->sons->sons->next->sons->next = NULL;
        o->effects->sons->sons->next->sons->sons = NULL;
        */

        // DEA stuff
        o->is_odd = FALSE;
        o->num_numeric_preconds_start = 0;
        o->num_preconds_overall = 0;
        o->num_preconds_end = 0;
        o->num_effects_start = 0;
        o->num_deleffects_start = 0;
        o->num_numeric_effects_end = 0;
        o->duration = NULL;
        // end of DEA stuff
...

{
  ...
  if ( gorig_initial_facts ) {
	...
	Hvar.common_values = (float *) realloc(Hvar.common_values, max_num_value * sizeof(float));
 	memset ( &Hvar.common_values[max_num_value - MAX_NUM_INC] , 0, MAX_NUM_INC * sizeof(float));
	...
	gis_inertial = (int*) realloc (gis_inertial, (max_num_value / 32 + 1) * sizeof(int));
	memset (&gis_inertial[ (max_num_value - MAX_NUM_INC) / 32 + 1], 0, (MAX_NUM_INC / 32 + 1) * sizeof(int));
     }
	...
	memset (&gis_inertial[ (max_num_value - MAX_NUM_INC) / 32 + 1], 0, (MAX_NUM_INC / 32 + 1) * sizeof(int));
      ...
      for (n = gorig_initial_facts->sons; n; n = n->next) {
	if (gnum_fullnum_initial == max_num_value) {
	  max_num_value += MAX_NUM_INC;
  ...
  for ( o = gloaded_ops; o; o = o->next ) {
    ...
    if (gcmd_line.display_info == 667) {
      printf("Plop %s\n", o->name);
      printf("num_numeric_preconds_start %d\n", o->num_numeric_preconds_start);
      printf("num_preconds_oval %d\n", o->num_preconds_overall);
      printf("num_preconds_end %d\n", o->num_preconds_end);
      printf("num_effects_start %d\n", o->num_effects_start);
      printf("num_deleffects_start %d\n",  o->num_deleffects_start);
      printf("num_numeric_effects_end %d\n", o->num_numeric_effects_end);
      if(o->duration) {printf("duration not null\n");}
      else {printf("duration is null\n");}
      printf("is_odd %d\n", o->is_odd);
      printf("number_of_real_params %d\n\n", o->number_of_real_params);
      printf("effect connective %d\n", o->effects->connective);
      printf("effect son connective %d\n", o->effects->sons->connective);
      printf("ss atom %d\n", o->effects->sons->sons->connective);
      printf("ssn atom %d\n", o->effects->sons->sons->next->connective);
    }                           
    // end of UIUC stuff   
  ...
  if ( 0 ) {
   ...
   /* derived predicates */
    gnum_deripreds /= 2;    /* !!!??? */
    gderipreds = (DerivePred *) malloc(sizeof(DerivePred)*gnum_deripreds);
    if (gcmd_line.display_info == 666)
        printf("Totally %d derived predicates\n", gnum_deripreds);
  for (dp=gloaded_dps;dp;dp=dp->next,i++) 
  {
    if (dp->preconds->connective == AND)
    {
        for (type_num=0,n=dp->preconds->sons;n;n=n->next)
            type_num++;
        gderipreds[i].num_prec = type_num;
        gderipreds[i].prec = (int *)
malloc(sizeof(int)*gderipreds[i].num_prec);   
        for (type_num=0,n=dp->preconds->sons;n;n=n->next,type_num++)
        {
            gderipreds[i].prec[type_num] =
position_in_predicates_table(n->atom->item);
            if (gderipreds[i].prec[type_num] == -1)
            {
                printf("Unknown predicates...\n");
                exit(1);
            }   
        }   
    }   
    ...
    {
        gderipreds[i].num_prec = 1;
        gderipreds[i].prec = (int *) malloc(sizeof(int));
        gderipreds[i].prec[0] =
position_in_predicates_table(dp->preconds->atom->item);
        if (gderipreds[i].prec[0] == -1)
        {
            printf("Unknown predicates...\n");
            exit(1);
        }   
    }   
    gderipreds[i].derived = position_in_predicates_table(dp->atom->item);
    if (gderipreds[i].derived == -1)
    {
        printf("Unknown predicates...\n");
        exit(1);
    }   
    ...
    {
        printf("%d: AND[ ", i);
        for (type_num=0;type_num<gderipreds[i].num_prec;type_num++)
            printf("%s ", gpredicates[gderipreds[i].prec[type_num]]);
        printf("] -> %s\n", gpredicates[gderipreds[i].derived]);
    }   
  } 

	/* timed initial literals */
	gtils = (TimedInitial *) realloc(gtils, sizeof(TimedInitial)*gnum_tils);
	for (i=0;i<gnum_tils;i++)
	{
		make_Fact(&(gtils[i].pred), gtils[i].literal, 0);
		if (gcmd_line.display_info == 666)
		{
			printf("\n%d: at %f NOT=%d ", i, gtils[i].time, gtils[i].negated);
			print_Fact(&(gtils[i].pred));
		}
	}
