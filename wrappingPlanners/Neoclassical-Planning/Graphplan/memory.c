...

{
  ...
  result->adds = NULL;
  result->adds_nondet = NULL;
  result->num_adds = 0;
  result->dels = NULL;
  result->dels_nondet = NULL;
  result->num_dels = 0;
...

{
  ...
  result->adds_nondet = ( Bool * ) calloc( e->num_adds, sizeof( Bool ) );
  result->num_adds = e->num_adds;
  for ( i = 0; i < e->num_adds; i++ ) {
    result->adds_nondet[i] = e->adds_nondet[i];
  ...
  result->dels_nondet = ( Bool * ) calloc( e->num_dels, sizeof( Bool ) );
  result->num_dels = e->num_dels;
  for ( i = 0; i < e->num_dels; i++ ) {
    result->dels_nondet[i] = e->dels_nondet[i];
...

{
  ...
  result->observe = op->observe;
  result->num_observe = op->num_observe;
...

{
  ...
  S->U = ( int * ) calloc( gnum_ft_conn, sizeof( int ) );
  S->num_U = 0;
...
BfsEdge *new_BfsEdge( void )

{

  BfsEdge *result = ( BfsEdge * ) calloc( 1, sizeof( BfsEdge ) );
  CHECK_PTR(result);

  result->unknown_E = ( int * ) calloc( gmax_E, sizeof( int ) );
  result->num_unknown_E = 0;

  result->in_node = NULL;
  result->out_node = NULL;
  result->true_out_node = NULL;
  result->false_out_node = NULL;
...

{
  ...
  result->ingoing_edge = NULL;
  result->outgoing_edges = NULL;
  result->best_outgoing_edge = NULL;
...
MemberList *new_MemberList( void )

{

  MemberList *result = ( MemberList * ) calloc( 1, sizeof( MemberList ) );
  CHECK_PTR(result);
