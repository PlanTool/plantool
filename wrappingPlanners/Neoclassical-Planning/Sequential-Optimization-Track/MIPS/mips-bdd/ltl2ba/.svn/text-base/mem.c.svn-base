...
static struct M *freelist[A_LARGE];
...
{	struct M *m;
	...
	u = (long) ((U-1)/sizeof(struct M) + 2);

	if (u >= A_LARGE)
	{	log(ALLOC, 0, 1);
	   if (tl_verbose) 
		printf("tl_spin: m memalloc %ld bytes\n", u);
	   m = (struct M *) emalloc((int) u*sizeof(struct M));
	   All_Mem += (unsigned long) u*sizeof(struct M);
	...
	{	if (!freelist[u])
		{	r = req[u] += req[u] ? req[u] : 1;
			...
			log(POOL, u, r);
			freelist[u] = (struct M *)
				emalloc((int) r*u*sizeof(struct M));
			All_Mem += (unsigned long) r*u*sizeof(struct M);
			m = freelist[u] + (r-2)*u;
		...
		m = freelist[u];
		freelist[u] = m->link;
                if ((int)m->link<0) {printf("ultimate case\n"); 
exit(1);} 
	}
	m->size = (u|A_USER);
