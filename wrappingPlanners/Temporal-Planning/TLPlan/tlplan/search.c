...
static void PruneUnreachable
(
	LINEARPLANP *pplpList,
	LINEARPLANP *pplpClosed
);
...
/* variables */
BOOL bReExpandSingleSuc=FALSE;   /* single successor reexpansion on by default */
double dfFathersHeuristic; // heuristic value of the father, used by delayed heuristic computation
LISTP plCurrentHelpfulActions; // helful actions for the node currently being expanded
int nHelpfulPreferredRounds=1; // number of rounds out of nHelpfulTotalRounds that a node is chosen from the helpful action list 
int nHelpfulTotalRounds=2;
...
int njbNodeCounter;  // node counter, used for debugging
...
)
{
	...
	for(i=0;plpCurrentState&&i<nLimit&&!bUserAbort;i++)
	{
		...
		if (bVerboseShowExpandedPlans) {
		  fprintf(HandleToFileStream(3),"[t=%lf] Expanding plan %d (h=%lf)",GetInternalRunTime(),njbNodeCounter,plpCurrentState->dfHeuristic);
		  if (pcMetricFn) {
		    double metric;
		    FormulaToDouble(pcMetricFn,plpCurrentState,pbBindings,&metric);
		    fprintf(HandleToFileStream(3),"-- metric=%lf\n",metric);
		  } else
		    fprintf(HandleToFileStream(3),"\n");
		  
		  PrintPlanList(plpCurrentState,HandleToFileStream(3));
		  fflush(HandleToFileStream(3));
		}
		if (bVerboseShowExpandedWorlds) {
		  fprintf(HandleToFileStream(3),"Expanded %d plan yields the following world\n",njbNodeCounter);
		  PrintWorld(3,plpCurrentState,pbBindings);
		}
...
)
{
	...
	ENTER("BestFirst",TRUE);
	phOpenHeap=CreateHeap();						/* create a new heap table for open list */
	...
	for(i=0;plpCurrentState&&i<nLimit&&!bUserAbort;i++)
	{
		...
		if (bVerboseShowExpandedPlans) {
		  ...
		  fprintf(HandleToFileStream(3),"[t=%lf] Expanding plan %d (h=%lf):\n",GetInternalRunTime(),njbNodeCounter,plpCurrentState->dfHeuristic);
		  fprintf(HandleToFileStream(3),"Heuristic Vector: [ ");
		  
		  for (j=0;j<plpCurrentState->nHeuristicVectorSize;j++) {
		    fprintf(HandleToFileStream(3),"%lf ",plpCurrentState->afHeuristicVector[j]);
		  }
		  fprintf(HandleToFileStream(3),"]\n");
		...
		if(bPruningAllSuccessors||!FalseFormQ(pcProgressedTLForm))
		{
				...
				(!dfSearchHeuristicLimit||plpCurrentState->dfHeuristic<=dfSearchHeuristicLimit))
			{
				...
				/* print heuristic value of children in the log file */
				//				if(plpChildrenStates)
				//  {
				//    for(plp=plpChildrenStates;plp;plp=plp->plpNext)
				//      fprintf(HandleToFileStream(3),"Heuristic of children = %lf,", plp->dfHeuristic);
				//} 
				...
				{
					for(plp=plpChildrenStates;plp;plp=plp->plpNext)
					  HeapInsert(phOpenHeap,plp);
				} 

				/* debugging, advice when current state had no successors */

				if(!plpChildrenStates&&bVerboseShowExpandedPlans)
				{
				   fprintf(HandleToFileStream(3),"NO successors for this plan");
				}
		...
		if(pzCurrent->nTotal>nZoneLimit)
		{
			...
			CommandPrintf(stderr,"\nCollecting garbage at world %d\n",nWorldNumber);
#else // _DEBUG
			CommandPrintf(stderr,"-\npzCurrent->nTotal=%d>%d\n",pzCurrent->nTotal,nZoneLimit);
#endif // _DEBUG
			MarkBindings(pbBindings);
			MarkDomainData();
			MarkPlanList(plpClosed);
		...
		plpCurrentState=HeapExtractMin(phOpenHeap);
	}
	...
	srSearchResult.nClosedLength=PlanCount(plpClosed);
#ifdef _DEBUG
	printf("Value of plpCurrentState: %p\n",plpCurrentState);
	printf("Open/Closed list size: %d/%d\n",srSearchResult.nOpenLength,srSearchResult.nClosedLength);
#endif
...
// A version of Best-First search with delayed heuristic computation
...
)
{
	...
	for(i=0;plpCurrentState&&i<nLimit&&!bUserAbort;i++)
	{
		...
		// compute the correct heuristic value for this state
		
		plpCurrentState->dfHeuristic=dfFathersHeuristic=CalculateHeuristic(pcHeuristicFn,plpCurrentState,pbBindings);
...
)
{
	...
	ENTER("BestFirstHelpfulList",TRUE);

	phStdHeap=CreateHeap();						/* create a new heap table for open list */
	phHelpfulHeap=CreateHeap();
	...
	for(i=0,nAlternate=0;plpCurrentState&&i<nLimit&&!bUserAbort;i++,nAlternate=(nAlternate+1)%nHelpfulTotalRounds)
	{
		...
		// compute the correct heuristic value for this state if it is not a preferred node
		
		if (!plpCurrentState->bIsPreferredNode)
		  plpCurrentState->dfHeuristic=CalculateHeuristic(pcHeuristicFn,plpCurrentState,pbBindings);
		
		dfFathersHeuristic=plpCurrentState->dfHeuristic;
		plCurrentHelpfulActions=plpCurrentState->plHelpfulActions;


		if (plpCurrentState->dfHeuristic<INFTY) {
		  ...
		  if (bVerboseShowExpandedPlans) {
		    int j; 
		    fprintf(HandleToFileStream(3),"[t=%lf] Expanding plan %d (h=%lf) (preferred=%d) (p=%p):\n",GetInternalRunTime(),njbNodeCounter,plpCurrentState->dfHeuristic,plpCurrentState->bIsPreferredNode,plpCurrentState);
		    fprintf(HandleToFileStream(3),"Heuristic Vector: [ ");
		    ...
		    fprintf(HandleToFileStream(3),"Best Relaxed Metric: %lf\n ",plpCurrentState->dfBestRelMetric);
		    
		    PrintPlanList(plpCurrentState,HandleToFileStream(3));
		    fflush(HandleToFileStream(3));
		  }
		  if (bVerboseShowExpandedWorlds) {
		    fprintf(HandleToFileStream(3),"Expanded plan %d yields the following world\n",njbNodeCounter);
		    PrintWorld(3,plpCurrentState,pbBindings);
		  }
		  ...
		  if(bPruningAllSuccessors||!FalseFormQ(pcProgressedTLForm))
				...
				(!dfSearchHeuristicLimit||plpCurrentState->dfHeuristic<=dfSearchHeuristicLimit))
			{
				...
				if (plpChildrenStates)
				  PruneUnreachable(&plpChildrenStates,&plpClosed);
				
				for(plp=plpChildrenStates;plp;plp=plp->plpNext) {
				  if (plp->bIsPreferredNode) {
				    HeapInsert(phHelpfulHeap,plp);
				  } else {
				    HeapInsert(phStdHeap,plp);
				  }
				}
		...
		else
		{
		  ...
		  /* current node determined unsolvable by heuristic, we don't want to alternate yet */
		  //nAlternate--;
		}
		...
		// select next world
		if (nAlternate<nHelpfulPreferredRounds) {
		  plpCurrentState=HeapExtractMin(phHelpfulHeap);
		  if (!plpCurrentState)
		    plpCurrentState=HeapExtractMin(phStdHeap);
		}
		else {
		  plpCurrentState=HeapExtractMin(phStdHeap);
		  if (!plpCurrentState)
...
)
{
	...
	int nSingleExpansions;
	
	const int LIMIT_SINGLE=40;
 ...
 start:
	...
	if (bReExpandSingleSuc && nSingleExpansions<LIMIT_SINGLE) {
	  if (plpSuccessors && !plpSuccessors->plpNext) {
	    // there is only one successor in the list
	    CELLP pc;
	    nSingleExpansions++;
	    pc=pcGoalFormula;
	    if (!(*pc->pfForm->paAction->pfEval)(pc,plpSuccessors,pbBindings)) {

	      // goal is not satisfied for this successor

	      //progress control formula
	      CELLP pcTLForm1;
	      CELLP pcProgressedTLForm;
	      int immutable; /* ignored */

	      if (bVerbose) printf("s");
	      // plpSuccessor becomes a real plan
	      plpSuccessors=PlanExtendNoHeuristic(plpSuccessors,pcTLForm,pcCCForm,plpPlan,pbBindings);
	      pcTLForm1=LinearPlanTLForm(plpSuccessors);
	      SetZone(&zScratch);
	      pcProgressedTLForm=(*pcTLForm->pfForm->paAction->pfProgress)
		(pcTLForm1,plpSuccessors,pbBindings,&immutable);
	      SetZone(&zPermanent);
	      ZoneCopyFormula(pcProgressedTLForm);
	      ZoneReloc((void **)&pcProgressedTLForm);
	      ZoneRelocFormula(pcProgressedTLForm);
	      ZoneClear(&zScratch);
	      // the progressed formula is not pcTLForm and our successor is plpPlan
	      pcTLForm=pcProgressedTLForm;
	      plpPlan=plpSuccessors;
	      
	      /* move current plan to the closed list */
	      plpPlan->plpNext=plpClosed;
	      plpClosed=plpPlan;
	...
	if (nSingleExpansions>=LIMIT_SINGLE) plpSuccessors=NULL; /* assuming a loop that will not reach the goal */
...
)
{
	...
	plp=(LINEARPLANP)MergeSort((MERGELISTP)plp,(MERGECOMPAREP)MergeCompareFn);
...
)
{
  int i,max;
	...
	/* on a tie, look at the heuristic vector (if it exists) */
	max=MAX(plpPlan1->nHeuristicVectorSize,plpPlan2->nHeuristicVectorSize);
	
	for (i=0; i<max; i++) {
	  if (plpPlan1->afHeuristicVector[i]<plpPlan2->afHeuristicVector[i]) {
	    EXIT("MergeCompareFn");
	    return -1;
	  } else if (plpPlan1->afHeuristicVector[i]>plpPlan2->afHeuristicVector[i]) {
	    EXIT("MergeCompareFn");
	    return  1;
	  }
	}
...
/* Extends MergeCompareNumericFn by looking into the heuristic vector */
...
)
{
  int i,max;
	...
	/* on a tie, look at the heuristic vector (if it exists) */
	max=MIN(plpPlan1->nHeuristicVectorSize,plpPlan2->nHeuristicVectorSize);
	...
	/* on a tie, test the plan length... shallower worlds sort earlier */
...
int MergeCompareNumericPreferLongFn
(
	LINEARPLANP plpPlan1,
	LINEARPLANP plpPlan2
)
...
)
{
  int i,max;
	...
	ENTER("MergePreferenceValueFirstFn",TRUE);

	/* Now the qualitative preference criterion */
	

	/* lexicographic ordering, starting from the highest prefernce value */
	for (i=nQualPreferenceNumber; i>0;i--) {
	  if (plpPlan1->afDistancesToPrefValues[i] < plpPlan2->afDistancesToPrefValues[i]) {
	    EXIT("MergePreferenceValueFirstFn");
	    return -1;
	  }
	...
	/* Now we continue with the original code... */
	...
	/* on a tie, test the plan length... "less deeper" worlds sort earlier */
...
{
  int i,argmin1,argmin2,max;
  

	ENTER("MergePreferenceEasyFirstFn",TRUE);

	/* Now the qualitative preference criterion */
	
	/* get the indices of the preferences that are easier (but harder than 0) */
	
	argmin1=argmin2=nQualPreferenceNumber;
	
	for (i=nQualPreferenceNumber; i>0; i--) {
	  if (plpPlan1->afDistancesToPrefValues[i]<plpPlan1->afDistancesToPrefValues[argmin1])
	    argmin1=i;
	  if (plpPlan2->afDistancesToPrefValues[i]<plpPlan2->afDistancesToPrefValues[argmin2])
	    argmin2=i;
	}
	

	/* compare the distance to the easiest preferences, and choose the easiest */

	if (plpPlan1->afDistancesToPrefValues[argmin1] < plpPlan2->afDistancesToPrefValues[argmin2]) {
	  EXIT("MergePreferenceEasyFirstFn");
	  return -1;
	}
	...
	/* still a tie, then choose the highest preference value */
...
)
{
  int i,max;
	...
	ENTER("MergeGoalPreferenceValueFirstFn",TRUE);

	/* first test the heuristic cost... cheaper worlds sort earlier */

	if(plpPlan1->dfHeuristic<plpPlan2->dfHeuristic)
	{
		EXIT("MergeGoalPreferenceValueFirstFn");
		return -1;
	}
	if(plpPlan1->dfHeuristic>plpPlan2->dfHeuristic)
	{
...
)
{
  int i,argmin1,argmin2,max;
	...
	ENTER("MergeGoalPreferenceEasyFirstFn",TRUE);

	/* first test the heuristic cost... cheaper worlds sort earlier */

	if(plpPlan1->dfHeuristic<plpPlan2->dfHeuristic)
	{
		EXIT("MergeGoalPreferenceEasyFirstFn");
		return -1;
	}
	if(plpPlan1->dfHeuristic>plpPlan2->dfHeuristic)
	{
...
{ 
  LINEARPLANP *plp;
  LINEARPLANP plpToClosed;
    
  plp=pplpList;
  while(*plp) {
    if ( (*plp)->dfHeuristic==INFTY ) {
      plpToClosed=*plp;
      *plp=(*plp)->plpNext;  // discarding (*plp) from list
      plpToClosed->plpNext=*pplpClosed;
      *pplpClosed=plpToClosed;
    } else 
      plp= &((*plp)->plpNext); // advancing plp pointer
  }
}  
