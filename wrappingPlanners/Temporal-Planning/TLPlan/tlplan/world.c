...
double dEvalTotalCost; /* the cost of evaluating a formulae
...
static HBTREEP *aphbtNewWorld;			/* new h-world pointer */
...
static HCELLP pcAddHinfo;            /* for handling lists with heuristic info */
static HCELLP pcAddHinfoEnd;
static HCELLP pcDelHinfo;
static HCELLP pcDelHinfoEnd;
...
static LISTP ArgTreeToList_H
(
	char *psName,						/* symbol name */
	HBTREEP pbtArgTree					/* its argument tree */
);
...
static LISTP ArgTreeToListTransform_H
(
	CELLP pcSymbol,
	HBTREEP pbtNode
);
...
HBTREEP *MakeWorld_H(void)
{
	...
	ENTER("MakeWorld",TRUE);
	if(nNumberOfDescribedSymbols)
		ppbt=(HBTREEP *)MemAlloc(sizeof(HBTREEP)*nNumberOfDescribedSymbols);
	else
		ppbt=NULL;
	EXIT("MakeWorld");
	return ppbt;
...
/* HCopyWorld is similar to CopyWorld, but: 
 1. Returns an array of HBTREEPs
 2. Does an actual copy of the binary trees 
*/
...
{
  int i;
  //  HBTREEP *aphbtNewWorld;
  ENTER("HCopyWorld",TRUE);
  
  aphbtNewWorld=(HBTREEP *)MemAlloc(sizeof(HBTREEP)*nNumberOfDescribedSymbols);
  
  for (i=0; i< nNumberOfDescribedSymbols; i++) {
    aphbtNewWorld[i]=BTree2HBTree(apbtWorld[i],aphbtNewWorld,i);
  }
  
  EXIT("HCopyWorld");
  return aphbtNewWorld;
}
...
)
{
	...
	plp->afDistancesToPrefValues=NULL;
	plp->afHeuristicVector=NULL;
	plp->nHeuristicVectorSize=0;
	plp->plHelpfulActions=NULL;
	plp->bIsPreferredNode=0; /* this is not a preferred node, until set otherwise */
	...
	if (GetInternalRunTime()>dPlanStopTime) 
	    AbortPlanner();
...
/* MakeWorldAction_H is the same as MakeWorldAction
   but the plan contains an h-world rather than a normal world */
...
//	double dfTime;
	...
	pcAddHinfo=NULL;
	pcAddHinfoEnd=(HCELLP)&pcAddHinfo;
	pcDelHinfo=NULL;
	pcDelHinfoEnd=(HCELLP)&pcDelHinfo;
...
)
{
	...
	apbtWorld=LinearPlanHWorld(plpLinearPlan);
	for(i=0;i<nNumberOfDescribedSymbols;i++)
	{
		nCount=HBTreeCount(GetSymbolArgs(apbtWorld,i),0);
		CommandPrintf(pfFile,"[%s, %d entr%s]",GetSymbolName(i),
			nCount,(nCount==1)?"y":"ies");
		PrintList(pfFile,ArgTreeToList_H(GetSymbolName(i),GetSymbolArgs(apbtWorld,i)),0);
		CommandPrintf(pfFile,"\n");
	}
	EXIT("PrintWorld");
    return TRUE;
...
/* same as InsertArgTree but for h-worlds*/

static HBTREEP InsertArgTree_H
(
	HBTREEP *apbtOwner,					// world database to update
	CELLP pcNewArg,						// list of arguments (i.e. key) to add
	HBTREEP pbtArgTree,					// pointer to btree to operate on
	CELLP pcValue,						// function value (optional)
	int nIndex,
	double cost,
	BOOL *inserted
)
{
	HBTREEP pbt;
	ENTER("InsertArgTree",TRUE);
	pbt=HBTreeInsert(apbtOwner,HMakeBTree(pcNewArg,pcValue,NULL,NULL,apbtOwner,nIndex,cost),
			 pbtArgTree,inserted);
	EXIT("InsertArgTree");
	return pbt;
}
...
static HBTREEP InsertArgTree_FF
(
	HBTREEP *apbtOwner,					// world database to update
	CELLP pcNewArg,						// list of arguments (i.e. key) to add
	HBTREEP pbtArgTree,					// pointer to btree to operate on
...
)
{
	...
	ENTER("InsertArgTree",TRUE);
	pbt=HBTreeInsert(apbtOwner,HMakeBTree_FF(pcNewArg,pcValue,NULL,NULL,apbtOwner,nIndex,cost,
						 poiOperInstance),
			 pbtArgTree,inserted);
	EXIT("InsertArgTree");
	return pbt;
...
static CELLP FindValueArgTree_H
(
	CELLP pcArg,
	HBTREEP pbtArgTree
)
{
	...
	ENTER("FindValueArgTree",TRUE);
	pbtNode=HBTreeSearch(pcArg,pbtArgTree);
	pc=pbtNode?pbtNode->pcValue:NULL;
	EXIT("FindValueArgTree");
	return pc;
}
...
)
{
	...
	cCell.pfForm=&fFormula;
	cCell.pcNext=NULL;
	pl=HBTreeToList(pbtArgTree,NULL,NULL,ArgTreeToListTransform_H,&cCell);
	EXIT("ArgTreeToList");
	return pl;
}
...
)
{
	...
	if(pcKey)
	{
		...
		sprintf(ac,"-with cost %3.1lf-",pbtNode->hsp_cost);
		//	if (pbtNode->hsp_cost!=0.0 && pbtNode->hsp_cost!=1.0 && pbtNode->hsp_cost!=2.0 && pbtNode->hsp_cost!=3.0 && pbtNode->hsp_cost!=4.0 && pbtNode->hsp_cost!=5.0 )
		//  printf("alert!! cost=%3.1lf\n",pbtNode->hsp_cost);
		
		plEnd=plEnd->plNext=NewList(ATOM_IDENT,ac); /* add the cost at the end of the list */
...
)
{
	...
	for(pc1=pcFormula->pfForm->pcArgs;pc1;pc1=pc1->pcNext) {
	  if (!(EqFormQ(pc1) || PlusEqFormQ(pc1) || MinusEqFormQ(pc1))) {
	    /* if it is a predicate...*/
	    
	    pc=pc1;
	    pcValue=NULL;
	    if(pc->pfForm->nType!=ATOM_SYMBOLINFOP) {
	      ErrorMessage("eval-add-hsp:  Not a symbol, \"%s\" ignored\n",
			   GetName(pc->pfForm,ac));
	      continue;
	    }
	    psiSymbolInfo=pc->pfForm->uValue.psiSymbolInfo;
	    nIndex=psiSymbolInfo-asiWorldSymbols;
	    if(nIndex<0||nIndex>=nNumberOfDescribedSymbols) {
	      /* if not described symbol */
	      ErrorMessage("eval-add:  Not a described symbol, \"%s\" ignored\n",
			   GetName(pc->pfForm,ac));
	      continue;
	    }
	    ...
	    /* Record the cost of adding this cell */

	    pch=(HCELLP)MemAlloc(sizeof(HCELL));
	
	    if (!bHSPIgnoreOperator) 
	      pch->hsp_cost = dEvalTotalCost;
	    else 
	      pch->hsp_cost = HSP_COST_FROM_IGNORED_OPERATOR;


	    pcAddHinfoEnd = pcAddHinfoEnd->pcNext = pch; 
...
)
{
	...
	/*	if (njbNodeCounter==25) {
	  fprintf(HandleToFileStream(3),"ADD list:");
	  }*/
	...
	for(pc1=pcFormula->pfForm->pcArgs;pc1;pc1=pc1->pcNext) {
	  if (!(EqFormQ(pc1) || PlusEqFormQ(pc1) || MinusEqFormQ(pc1))) {
	    ...
	    /* if (njbNodeCounter==25) {
	      PrintFormula(HandleToFileStream(3),pc,0);
	      }*/
	    ...
	    // check whether current instance is all we need...
	    if (poiCurrentInstance&&poiCurrentInstance->pcSupFacts==pcFFSupFacts) {
	      pch->poiOperInstance=poiCurrentInstance;
	    } else {
	      poiCurrentInstance=pch->poiOperInstance=(OPERINSTANCEP)MemAlloc(sizeof(OPERINSTANCE));
	      
	      /* now ff-specific info */
	      
	      //     printf("pcFFSupFacts=%p\n",pcFFSupFacts);
	      
	      pch->poiOperInstance->pbBindings=pbCurrentActionBindings;
	      pch->poiOperInstance->pcSupFacts=pcFFSupFacts;
	      pch->poiOperInstance->nOperNumber=nFFOperNumber;
	      pch->poiOperInstance->nOperInstance=nFFOperInstance;
	      pch->poiOperInstance->nDepth=nFFDepth;
	      pch->poiOperInstance->pcActionInstance=pcFFActionInstance;
	      pch->poiOperInstance->dHspCost=dEvalTotalCost;
	      pch->poiOperInstance->nAddedFacts=0;
	      pch->poiOperInstance->plFlipUndo=NULL;
	      pch->poiOperInstance->plFlipUndoMin=NULL;
	    }
	    //	    fprintf(HandleToFileStream(3),":::Add: nFFDepth=%d\n",pch->nFFDepth);
...
)
{
	...
	for(pc1=pcFormula->pfForm->pcArgs;pc1;pc1=pc1->pcNext)
	{
		...
		pcDelHinfoEnd=pcDelHinfoEnd->pcNext=pch;
...
)
{
	...
	/*
	if (njbNodeCounter==25) {
	  fprintf(HandleToFileStream(3),"DEL list:");
	  } */
	...
	for(pc1=pcFormula->pfForm->pcArgs;pc1;pc1=pc1->pcNext)
	{
	  ...
	  /*	  if (njbNodeCounter==25) {
	    PrintFormula(HandleToFileStream(3),pc1,0);
	    } */
...
BOOL EvalNODel  /* Dummy del function that does nothing */
(
	CELLP pcFormula,					/* del formula */
	LINEARPLANP plpLinearPlan,			/* current plan */
	BINDINGP pbBindings					/* current bindings */
...
void UpdateWorld(void)
{
  ...
  for(pc=pcDel;pc;pc=pc->pcNext)
  {
    ...
    else if (aModifyWorldAction.pfEval != EvalModifyWorldRelaxed)   // do not show the warning if we are in relaxed mode
    {
  ...
  for(pc1=pcAdd;pc1;pc1=pc1->pcNext)
  {
    ...
    if(BTreeSearch(pcArgs,pbtArgTree))
    {
      if (0&&bVerbose && PredicateQ(psiSymbolInfo)&&!psiSymbolInfo->bRewritable && aDelAction.pfEval!=EvalNODel )
      {
...
Description:
   ...
   Called only from relax plan mode, so simplify the logic.
*/
...
int AddsFromUpdateWorld(void)
{
 ...
 for(pc1=pcDel;pc1;pc1=pc1->pcNext)
  {
    if(EqFormQ(pc1)||PlusEqFormQ(pc1)||MinusEqFormQ(pc1))	/* if this is a function */
    {
    ...
    if(!psiSymbolInfo)
    {
      Message("eval-add:  Cannot add non-described symbol \"%s\", addition ignored.\n",
	      GetName(pc->pfForm,ac));
      PrintFormula(pfError,pc1,0);
      continue;
    }
    ...
    nIndex=psiSymbolInfo-asiWorldSymbols;
    pbtArgTree=GetSymbolArgs(apbtNegWorld,nIndex);
    pcArgs=pc->pfForm->pcArgs;
    if(BTreeSearch(pcArgs,pbtArgTree))
    {
      //fact or function value already in btree. Ignore fact.
      if(PredicateQ(psiSymbolInfo))
	continue;
      else
      {
	continue;
	//Function value---should we ignore? Yes for now. 
	/*pbtArgTree=BTreeDelete(apbtNegWorld,pcArgs,pbtArgTree);
	PutSymbolArgs(apbtNegWorld,nIndex,
	InsertArgTree(apbtNegWorld,pcArgs,pbtArgTree,pcValue));*/
      }
    }
    else {
      PutSymbolArgs(apbtNegWorld,nIndex,
		    InsertArgTree(apbtNegWorld,pcArgs,pbtArgTree,pcValue));
      ++numAdds; //put this inside of else. 
    }
  }
  ...
  /* then apply the adds */
  /* Perhaps not worthwhile---omit for now*/
  /*try to keep the btrees balanced */
  //pcAdd=PermuteFormulaList(pcAdd);
...
/* The H-version of AddsFromUpdateWorld */



int AddsFromUpdateWorld_H(double action_cost)
{
  CELLP pc,pc1;
  HCELLP pch;
  CELLP pcValue;
  ...
  for(pc1=pcDel,pch=pcDelHinfo;pc1;pc1=pc1->pcNext,pch=pch->pcNext)
  {

    //    fprintf(HandleToFileStream(3),"deleting: ");
    //PrintFormula( HandleToFileStream(3),pc1,0);

    if(EqFormQ(pc1)||PlusEqFormQ(pc1)||MinusEqFormQ(pc1))	/* if this is a function */
    {
      /* there should not be functions here! */
      pc=pc1->pfForm->pcArgs;
      pcValue=pc->pcNext;		/* += and -= have already been taken care of */
    }
    else							/* must be a predicate */
    ...
    nIndex=psiSymbolInfo-asiWorldSymbols;
    pbtArgTree=GetSymbolArgs(aphbtNegWorld,nIndex);
    pcArgs=pc->pfForm->pcArgs;
    

    if(abUnRelaxableSymbols[nIndex]) {
      /* this is not a relaxable symbol, we effectively delete it from the h-world */
       PutSymbolArgs(aphbtNewWorld,nIndex,
		     HBTreeDelete(aphbtNewWorld,pcArgs,pbtArgTree));
      //      printf("here %d\n",nIndex);
    } else {


      if (pch->hsp_cost != HSP_COST_FROM_IGNORED_OPERATOR)
	/* add comes from an operator that is ignored */
	phbtNewTree = InsertArgTree_H(aphbtNegWorld,pcArgs,pbtArgTree,pcValue,nIndex,action_cost+pch->hsp_cost,&inserted);
      else 
	phbtNewTree = InsertArgTree_H(aphbtNegWorld,pcArgs,pbtArgTree,pcValue,nIndex,pch->hsp_cost,&inserted);

      PutSymbolArgs(aphbtNegWorld,nIndex,phbtNewTree);
      // PutSymbolArgs(aphbtNegWorld,nIndex,
      //		  InsertArgTree_H(aphbtNegWorld,pcArgs,pbtArgTree,pcValue,nIndex,1+pch->hsp_cost,&inserted));
      
      if (inserted&&!abUnRelaxableSymbols[nIndex]) // only increment the counter when symbol is relaxed 
      //      if (inserted) // a negative fact has been inserted into the tree
	++numAdds;
    }
  }
  ...
  pcDelHinfo=NULL; // JB: I don't like how lists are managed. Keeping style.
  pcDelHinfoEnd=(HCELLP)&pcDelHinfo;
  ...
  for(pc1=pcAdd,pch=pcAddHinfo;pc1;pc1=pc1->pcNext,pch=pch->pcNext)
  {
    //    fprintf(HandleToFileStream(3),"adding: ");
    //PrintFormula(HandleToFileStream(3),pc1,0);
    ...
    nIndex=psiSymbolInfo-asiWorldSymbols;
    pbtArgTree=GetSymbolArgs(aphbtNewWorld,nIndex);
    pcArgs=pc->pfForm->pcArgs;
    ...
    if (pch->hsp_cost != HSP_COST_FROM_IGNORED_OPERATOR) 
      PutSymbolArgs(aphbtNewWorld,nIndex,
		    InsertArgTree_H(aphbtNewWorld,pcArgs,pbtArgTree,pcValue,nIndex,action_cost+pch->hsp_cost,&inserted));
    else 
      PutSymbolArgs(aphbtNewWorld,nIndex,
		    InsertArgTree_H(aphbtNewWorld,pcArgs,pbtArgTree,pcValue,nIndex,pch->hsp_cost,&inserted));


    if (inserted&&!abUnRelaxableSymbols[nIndex]) // only increment the counter when symbol is relaxed 
    //    if (inserted) // increment the counter if something inserted
      ++numAdds;
  ...
  pcAddHinfo=NULL; // JB: I don't like how lists are managed. Keeping style.
  pcAddHinfoEnd=(HCELLP)&pcAddHinfo;
...
int AddsFromUpdateWorld_H_unreleffs(double action_cost,int *actual_adds)
/* unrelaxed effects */
{
  CELLP pc,pc1;
  HCELLP pch;
  ...
  *actual_adds = 0;
  ...
  for(pc1=pcDel,pch=pcDelHinfo;pc1;pc1=pc1->pcNext,pch=pch->pcNext)
  {
    ...
    if(abUnRelaxableSymbols[nIndex]) {
      ...
      if (abHSPIgnoredSymbols[nIndex]) /* cost of this symbol should be always 0 */
	phbtNewTree = InsertArgTree_H(aphbtNegWorld,pcArgs,pbtArgTree,pcValue,nIndex,0,&inserted);
      else if (pch->hsp_cost != HSP_COST_FROM_IGNORED_OPERATOR)
	/* add comes from an operator that is ignored */
	phbtNewTree = InsertArgTree_H(aphbtNegWorld,pcArgs,pbtArgTree,pcValue,nIndex,action_cost+pch->hsp_cost,&inserted);
      else 
	phbtNewTree = InsertArgTree_H(aphbtNegWorld,pcArgs,pbtArgTree,pcValue,nIndex,pch->hsp_cost,&inserted);
      ...
      if (inserted) // only increment the counter when symbol is relaxed 
      //      if (inserted) // a negative fact has been inserted into the tree
	++numAdds;
    }
  }
  ...
  for(pc1=pcAdd,pch=pcAddHinfo;pc1;pc1=pc1->pcNext,pch=pch->pcNext)
  {
    ...
    if (abHSPIgnoredSymbols[nIndex])
      PutSymbolArgs(aphbtNewWorld,nIndex,
		    InsertArgTree_H(aphbtNewWorld,pcArgs,pbtArgTree,pcValue,nIndex,0,&inserted));
    else if (pch->hsp_cost != HSP_COST_FROM_IGNORED_OPERATOR) 
      PutSymbolArgs(aphbtNewWorld,nIndex,
		    InsertArgTree_H(aphbtNewWorld,pcArgs,pbtArgTree,pcValue,nIndex,action_cost+pch->hsp_cost,&inserted));
    else 
    ...
    if (inserted) // increment the counter of actual additions
      ++(*actual_adds);
...
int AddsFromUpdateWorld_FF_unreleffs(double action_cost,int *actual_adds)
/* unrelaxed effects */
{
  CELLP pc,pc1;
  HCELLP pch;
  ...
  *actual_adds = 0;
  ...
  for(pc1=pcDel,pch=pcDelHinfo;pc1;pc1=pc1->pcNext,pch=pch->pcNext)
  {
    ...
    if(abUnRelaxableSymbols[nIndex]) {
      ...
      if (abHSPIgnoredSymbols[nIndex]) /* cost of this symbol should be always 0 */
	phbtNewTree = InsertArgTree_FF(aphbtNegWorld,pcArgs,pbtArgTree,pcValue,nIndex,0,pch->poiOperInstance,&inserted);
      else if (pch->hsp_cost != HSP_COST_FROM_IGNORED_OPERATOR)
	/* add comes from an operator that is ignored */
	phbtNewTree = InsertArgTree_FF(aphbtNegWorld,pcArgs,pbtArgTree,pcValue,nIndex,action_cost+pch->hsp_cost,pch->poiOperInstance,&inserted);
      else 
	phbtNewTree = InsertArgTree_FF(aphbtNegWorld,pcArgs,pbtArgTree,pcValue,nIndex,pch->hsp_cost,pch->poiOperInstance,&inserted);

      PutSymbolArgs(aphbtNegWorld,nIndex,phbtNewTree);
      // PutSymbolArgs(aphbtNegWorld,nIndex,
      //		  InsertArgTree_H(aphbtNegWorld,pcArgs,pbtArgTree,pcValue,nIndex,1+pch->hsp_cost,&inserted));

    if (inserted&&!abUnRelaxableSymbols[nIndex]&&!abHSPIgnoredSymbols[nIndex]) // only increment the counter when symbol is relaxed 
    //    if (inserted) // increment the counter if something inserted
      ++numAdds;
    }
  }
  ...
  for(pc1=pcAdd,pch=pcAddHinfo;pc1;pc1=pc1->pcNext,pch=pch->pcNext)
  {
    ...
    if (abHSPIgnoredSymbols[nIndex])
      PutSymbolArgs(aphbtNewWorld,nIndex,
		    InsertArgTree_FF(aphbtNewWorld,pcArgs,pbtArgTree,pcValue,nIndex,0,pch->poiOperInstance,&inserted));
    else if (pch->hsp_cost != HSP_COST_FROM_IGNORED_OPERATOR) 
      PutSymbolArgs(aphbtNewWorld,nIndex,
		    InsertArgTree_FF(aphbtNewWorld,pcArgs,pbtArgTree,pcValue,nIndex,action_cost+pch->hsp_cost,pch->poiOperInstance,&inserted));
    else 
      PutSymbolArgs(aphbtNewWorld,nIndex,
		    InsertArgTree_FF(aphbtNewWorld,pcArgs,pbtArgTree,pcValue,nIndex,pch->hsp_cost,pch->poiOperInstance,&inserted));
    ...
    if (inserted&&!abUnRelaxableSymbols[nIndex]&&!abHSPIgnoredSymbols[nIndex]) 
      // only increment the counter when symbol is relaxed and it is not ignored (real-world)
    //    if (inserted) // increment the counter if something inserted
      ++numAdds;
...
/* A version of AddsFromUpdateWorld that is used for computing occlusion penalties */

int AddsFromUpdateExecute(double action_cost,int *actual_adds)
/* unrelaxed effects */
{
  CELLP pc,pc1;
  HCELLP pch;
  ...
  *actual_adds = 0;
  ...
  for(pc1=pcDel,pch=pcDelHinfo;pc1;pc1=pc1->pcNext,pch=pch->pcNext)
  {
    ...
    if(abUnRelaxableSymbols[nIndex]) {
      ...
      // nothing relative to occulusions is computed here
      ...
      pbtArgTree=GetSymbolArgs(aphbtNegWorld,nIndex);
      pcArgs=pc->pfForm->pcArgs;
      //      fprintf(HandleToFileStream(3),"Execute DEL at depth %d nFFDepth=%d\n",pch->nFFDepth,nFFDepth);
      if (abHSPIgnoredSymbols[nIndex]) /* cost of this symbol should be always 0 */
	phbtNewTree = InsertArgTree_FF(aphbtNegWorld,pcArgs,pbtArgTree,pcValue,nIndex,0,pch->poiOperInstance,&inserted);
      else if (pch->hsp_cost != HSP_COST_FROM_IGNORED_OPERATOR)
	/* add comes from an operator that is ignored */
	...
	phbtNewTree = InsertArgTree_FF(aphbtNegWorld,pcArgs,pbtArgTree,pcValue,nIndex,pch->hsp_cost,pch->poiOperInstance,&inserted);
      ...
      /* now, occlude the positive fact, if any */
      if ( (phbtComplementTree=HBTreeSearch(pcArgs,GetSymbolArgs(aphbtNewWorld,nIndex))) ) {
	
	if (phbtComplementTree->nDepthAdded<nFFDepth) {
	  //fprintf(HandleToFileStream(3),"Occluding "); PrintFormula(HandleToFileStream(3),pc1,0);
	  phbtComplementTree->bNotOccluded=0;  // occluding positive fact
	}
      }			
  ...
  for(pc1=pcAdd,pch=pcAddHinfo;pc1;pc1=pc1->pcNext,pch=pch->pcNext)
  {
    ...
    inserted=0;
    //    fprintf(HandleToFileStream(3),"Execute ADD at depth %d nFFDepth=%d\n",pch->nFFDepth,nFFDepth);
    if (abHSPIgnoredSymbols[nIndex])
      phbtNewTree=InsertArgTree_FF(aphbtNewWorld,pcArgs,pbtArgTree,pcValue,nIndex,0,pch->poiOperInstance,&inserted);
    else if (pch->hsp_cost != HSP_COST_FROM_IGNORED_OPERATOR) 
      phbtNewTree=InsertArgTree_FF(aphbtNewWorld,pcArgs,pbtArgTree,pcValue,nIndex,action_cost+pch->hsp_cost,pch->poiOperInstance,&inserted);
    else 
      phbtNewTree=InsertArgTree_FF(aphbtNewWorld,pcArgs,pbtArgTree,pcValue,nIndex,pch->hsp_cost,pch->poiOperInstance,&inserted);

    
    PutSymbolArgs(aphbtNewWorld,nIndex,phbtNewTree);

    /* now, occlude the negative fact */
    if ( (phbtComplementTree=HBTreeSearch(pcArgs,GetSymbolArgs(aphbtNegWorld,nIndex))) ) {
      
      if (phbtComplementTree->nDepthAdded<nFFDepth) {
	//	fprintf(HandleToFileStream(3),"Occluding not-"); PrintFormula(HandleToFileStream(3),pc1,0);
	phbtComplementTree->bNotOccluded=0;  // occluding positive fact
      }
    } else if (inserted) {    // fact was not in negated tree and was just inserted 
      phbtNewTree=InsertArgTree_FF(aphbtNegWorld,pcArgs,GetSymbolArgs(aphbtNegWorld,nIndex),pcValue,nIndex,pch->hsp_cost,pch->poiOperInstance,&inserted);
      PutSymbolArgs(aphbtNegWorld,nIndex,phbtNewTree);
      phbtNewTree->bNotOccluded=0;
      phbtNewTree->nDepthAdded=nFFDepth-1;  // simulate addition in the previous level
    }
...
{
    if (bUnderNot)  /* there is a not preceding this formula */
	return (EvalDescPredicateStd(pcFormula,plpLinearPlan,pbBindings) && 
		!EvalDescPredicateStd(pcFormula,plpNegPlan,pbBindings));
    else 
	return EvalDescPredicateStd(pcFormula,plpLinearPlan,pbBindings);
}
...
)
{
	...
	if (foundNode) 
	  dEvalTotalCost+=foundNode->hsp_cost;
...
)
{
  BOOL b;
  if (bUnderNot) { /* there is a not preceding this formula */
    double cost_before = dEvalTotalCost;

    b = (!EvalDescPredicateStd_H(pcFormula,plpNegPlan,pbBindings) && 
	 EvalDescPredicateStd_H(pcFormula,plpLinearPlan,pbBindings));
    
    if (b)
      dEvalTotalCost = cost_before; /* if the formula will finally evaluate to false
				       then do not count the cost of the evaluation */

  }
  else 
    b = EvalDescPredicateStd_H(pcFormula,plpLinearPlan,pbBindings);
...
BOOL EvalDescPredicateStd_FF /* modification of the original version of EvalDescPredicate */
(
	CELLP pcFormula,
	LINEARPLANP plpLinearPlan,
	BINDINGP pbBindings
)
{
	...
	//	if (njbNodeCounter==2) {
	//  int i;
	//  fprintf(HandleToFileStream(3),"Called eval with: ");
	//  for (i=0; i<asiWorldSymbols[nIndex].nArity; i++) {
	//    PrintFormula(HandleToFileStream(3),pcTerms+i,0);
	//  }
	//}
	...
	if (foundNode) {
	  LISTP pcFFNewHead;
	  dEvalTotalCost+=foundNode->hsp_cost;
	  if (bComputingOcclusions && foundNode->bNotOccluded==0) {
	    if (bVerboseFFShowRelaxedPlans) {
	      CELLP pc3;
	      pc3=(CELLP)MemAlloc(sizeof(CELL));
	      pc3->pfForm=(FORMULAP)CopyAlloc(pcFormula->pfForm,sizeof(FORMULA));
	      if (pcFormula->pfForm->pcArgs) pc3->pfForm->pcArgs=ComputeTerms(pcFormula->pfForm->pcArgs,plpLinearPlan,pbBindings);
	      fprintf(HandleToFileStream(3),"Occluded fact at level %d:",nFFDepth-1);
	      PrintFormula(HandleToFileStream(3),pc3,0);
	    }
	    nOcclusionPenalty++;
	    // now un-occluding it
	    foundNode->bNotOccluded=1;
	    foundNode->nDepthAdded=nFFDepth-1;
	  }
	  if (!abUnRelaxableSymbols[nIndex] && !abHSPIgnoredSymbols[nIndex]) {
	    /* Updating plFFSupFacts */
	    pcFFNewHead=(LISTP)MemAlloc(sizeof(LIST));
	    pcFFNewHead->uValue.phbtTree=foundNode;
	    pcFFNewHead->plNext=pcFFSupFacts;
	    pcFFSupFacts=pcFFNewHead;
	  }
...
BOOL EvalDescPredicateStd_VP /* modification of the original version of EvalDescPredicate */
(
	CELLP pcFormula,
	LINEARPLANP plpLinearPlan,
	BINDINGP pbBindings
)
{
	...
	b=(BOOL)foundNode;
	//	if (b) printf("true, in=%d\n",foundNode->nIndex);
	if (foundNode && foundNode->nDepthAddedByRelaxed<0) {
	  //printf("true but not true :)\n");
	  b=0;  /* node is false if it has not been added by relaxed plan */
	}
	EXIT("EvalDescPredicateStd");
	return b;
}
...
BOOL EvalDescPredicateStd_FF_MM /* Version of EvalDescPredicate that computes mutex penalties */
(
	CELLP pcFormula,
	LINEARPLANP plpLinearPlan,
	LINEARPLANP plpComplementLinearPlan,
	BINDINGP pbBindings
)
{
	CELL pcTerms[50];
	...
	if (foundNode&&foundNode->nIndex==0) { // only count mutexes on one fluent
	//if (foundNode) { 
	  HBTREEP complementNode;
	  complementNode=HBTreeSearch(pcTerms,plpComplementLinearPlan->aphbtWorld[nIndex]);
	  if (complementNode) {
	    LISTP pl;
	    if (bVerboseFFShowRelaxedPlans)
	      fprintf(HandleToFileStream(3),"Possible mutex for action (%d,%d) complement node is %p\n", 
		      nFFOperNumber,
		      nFFOperInstance,
		      complementNode);
		    
	    /* check for a possible mutex */
	    for (pl=complementNode->plActionList; pl; pl=pl->plNext) {
	      if (bVerboseFFShowRelaxedPlans) fprintf(HandleToFileStream(3),"-- Looking at (%d,%d) at depth %d ", 
						      pl->uValue.poiOperInstance->nOperNumber,
						      pl->uValue.poiOperInstance->nOperInstance,
						      pl->uValue.poiOperInstance->nDepth);

	      if (pl->uValue.poiOperInstance->nDepth==nFFDepth &&
		  !(pl->uValue.poiOperInstance->nOperNumber==nFFOperNumber&&
		    pl->uValue.poiOperInstance->nOperInstance==nFFOperInstance)) {
		if (bVerboseFFShowRelaxedPlans) fprintf(HandleToFileStream(3),"mutex counted\n");
		++nMutexPenalty; // increment mutex counter
	      } else {
		if (bVerboseFFShowRelaxedPlans) fprintf(HandleToFileStream(3),"mutex not counted\n");
	      }
	      if (pl->uValue.poiOperInstance->nDepth<nFFDepth) break; // stop the search if we've seen all actions at this level
	    }
	  }
...
)
{
  ...
  if (bUnderNot) { /* there is a not preceding this formula */
    double cost_before = dEvalTotalCost;
    LISTP pcFFSupFacts_before=pcFFSupFacts;
    
    
    if (!abUnRelaxableSymbols[nIndex]) {
      /* relaxed symbol */
      b = (!EvalDescPredicateStd_FF(pcFormula,plpNegPlan,pbBindings) &&
	   EvalDescPredicateStd(pcFormula,plpLevel0Plan,pbBindings));
    } else {
      /* unrelaxed symbol */
      b = EvalDescPredicateStd_FF(pcFormula,plpLinearPlan,pbBindings);
    }
	...
	dEvalTotalCost = cost_before; /* if the formula will finally evaluate to false
      ...
      pcFFSupFacts=pcFFSupFacts_before; /* restoring the list */
...
BOOL EvalDescPredicateRelaxedNeg_FF_MM
(
	CELLP pcFormula,
	LINEARPLANP plpLinearPlan,
	BINDINGP pbBindings
)
{
  ...
  if (bUnderNot) { /* there is a not preceding this formula */
    ...
    if (!abUnRelaxableSymbols[nIndex]) {
      /* relaxed symbol */
      b = (!EvalDescPredicateStd_FF_MM(pcFormula,plpNegPlan,plpLinearPlan,pbBindings) &&
	   EvalDescPredicateStd(pcFormula,plpLevel0Plan,pbBindings));
    } else {
      /* unrelaxed symbol */
      b = EvalDescPredicateStd_FF_MM(pcFormula,plpLinearPlan,plpNegPlan,pbBindings);
    }
...
)
{
  ...
  if (bUnderNot) { /* there is a not preceding this formula */
    ...
    if (!abUnRelaxableSymbols[nIndex]) {
      /* relaxed symbol */
      b = (!EvalDescPredicateStd_VP(pcFormula,plpNegPlan,pbBindings) &&
	   EvalDescPredicateStd(pcFormula,plpLevel0Plan,pbBindings));
    } else {
      /* unrelaxed symbol */
      b = EvalDescPredicateStd_VP(pcFormula,plpLinearPlan,pbBindings);
    }
...
{
  if (bMutexMode)
    return EvalDescPredicateRelaxedNeg_FF_MM(pcFormula,plpLinearPlan,pbBindings);
  if (bRelaxedModeNeg)
      return EvalDescPredicateRelaxedNeg(pcFormula,plpLinearPlan,pbBindings);
  if (bUseHWorld)
    return EvalDescPredicateRelaxedNeg_H(pcFormula,plpLinearPlan,pbBindings);
  if (bUseHWorldFF)
    return EvalDescPredicateRelaxedNeg_FF(pcFormula,plpLinearPlan,pbBindings);
  if (bExtractingRelaxedPlan)
    return EvalDescPredicate_Extract_Relaxed_Plan(pcFormula,plpLinearPlan,pbBindings);
  if (bVerifyingPreservedProperty)
    return EvalDescPredicate_VP(pcFormula,plpLinearPlan,pbBindings);
  return EvalDescPredicateStd(pcFormula,plpLinearPlan,pbBindings);
}
...
{
  if (bRelaxedModeNeg || bUseHWorld || bUseHWorldFF || bExtractingRelaxedPlan)
    return ComputeDescFunction_H(pcFormula,plpLinearPlan,pbBindings);
  else
    return ComputeDescFunction_Std(pcFormula,plpLinearPlan,pbBindings);
}
...
/*
  ComputeDescFunction_H same as ComputeDescFunction but uses H-worlds
*/
...
)
{
	...
	if(!plpLinearPlan->aphbtWorld[nIndex])	// if function isn't initialized
		return 0;
	pc=FindValueArgTree_H(pcTerms,plpLinearPlan->aphbtWorld[nIndex]);
	pc1=CopyCell(pc);
	EXIT("ComputeDescFunction");
	return pc1;
}
